...
describe("PitTracker", () => {
  ...
  test("ignores tire wear when active game cannot estimate wear", () => {
    const tracker = new PitTracker();
    tracker.setTireWearAvailable(false);
    tracker.feed(pkt({ LapNumber: 1, Fuel: 1, CurrentLap: 0 }), 5000);
    completeLap(tracker, 1, { fuel: 0.9, wearFL: 0.1, wearFR: 0.1, wearRL: 0.1, wearRR: 0.1 });

    const result = tracker.feed(pkt({ LapNumber: 2, Fuel: 0.9, TireWearFL: 0.1, TireWearFR: 0.1, TireWearRL: 0.1, TireWearRR: 0.1, CurrentLap: 5 }), 5000);
    expect(result.tireLapsToBad).toBeNull();
    expect(result.tireLapsToCritical).toBeNull();
    expect(result.limitedBy).toBe("fuel");
    expect(result.pitInLaps).toBeCloseTo(9, 0);
  });
