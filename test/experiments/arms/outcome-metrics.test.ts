 ...
 * equivalence test in test/experiments/arms/stream-equivalence.test.ts cannot be
 * it: both paths call `pickReferenceLap`, so a change to the rule moves both sides together and
...
describe("curation policy is per-metric", () => {
  test("no shipped metric ranks laps away — every one is all-valid + blunder fence", () => {
    ...
    // statistics instead of an iid sample. test/experiments/arms/outcomes.test.ts measures
