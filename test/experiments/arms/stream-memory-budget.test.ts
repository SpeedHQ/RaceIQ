...
import { FRAME_BUDGET_PER_ARM, selectWithinFrameBudget, streamArmSamples } from "../../../server/experiments/comparison/stream";
...
describe("peak live semantic samples is 2 laps", () => {
  test("each lap replays exactly once and never survives its reduce", async () => {
    ...
    const streamed = await streamArmSamples({
      ...
      loadSamples: tracker.loadSamples,
      resolveCorners: async () => CORNERS,
    });

    // One replay per lap: reference first, then every other lap once.
    expect(tracker.decoded.length).toBe(SCATTERED.length);
    expect(new Set(tracker.decoded).size).toBe(SCATTERED.length);
    // Reference plus lap in hand; nothing accumulates.
  ...
  test("a metric never sees a third lap: reduce receives exactly two", async () => {
    const built = buildStreamingArm(SCATTERED);
    const seen: number[] = [];
    const spy: PairwiseFramesOutcomeMetric = {
      ...
      reduce: (input) => {
        ...
        expect(input.samples).not.toBe(input.referenceSamples);
  ...
  test("a budget that trims REPORTS the drop; it never silently truncates", async () => {
    ...
    const cmp = compareArmSamples(
      ...
      await streamArmSamples({
        ...
        metric,
        loadSamples: trackingLoader(buildStreamingArm(REPEATABLE, 100).frames).loadSamples,
        resolveCorners: async () => CORNERS,
      }),
      metric,
    );
