...
describe("telemetry fidelity vs MoTeC-rate logging", () => {
  ...
  // every poll. See docs/research/telemetry-fidelity.md sections 1 and 6.
  ...
  test("a third of frames re-read an unchanged graphics page", () => {
    ...
    // size win available (docs/research/telemetry-fidelity.md section 6).
  ...
  test("poll spacing is one Windows timer tick, not the 10ms we ask for", () => {
    ...
    const pid = allPackets.map((p) => p.acc?.acEvo?.physicsPacketId).filter((v): v is number => typeof v === "number");
    const deltas: number[] = [];
  ...
  describe("what does not survive decimation to 26.6Hz", () => {
    ...
    test("random-phase miss probability at 26.6Hz is ~5%, not the 14% short-event share", () => {
      const durations = eventDurationsMs((p) => Math.abs(p.TireSlipRatioRL) > 0.05 || Math.abs(p.TireSlipRatioRR) > 0.05);
