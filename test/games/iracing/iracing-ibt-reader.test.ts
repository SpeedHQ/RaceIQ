...
describe("IRacingIbtReader", () => {
  test("streams SDK-compatible snapshots and exposes disk metadata", async () => {
    ...
    try {
      ...
      expect(first?.values.Pitch).toBeCloseTo(0.1);
      expect(first?.values.Roll).toBeCloseTo(-0.2);
  ...
  test("reuses the existing source-frame and normalizer path", async () => {
    ...
    try {
      ...
      expect(delivered[1].length).toBeLessThan(delivered[0].length / 5);
      ...
      expect(packet.Pitch).toBeCloseTo(0.1);
      expect(packet.Roll).toBeCloseTo(-0.2);
      ...
      expect(secondPacket.Pitch).toBeCloseTo(0.2);
      expect(secondPacket.Roll).toBeCloseTo(-0.3);
