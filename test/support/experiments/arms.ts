...
import type { FrameLapMeta, SemanticSampleLoader } from "../../../server/experiments/comparison/stream";
...
/** Straight-line semantic lap (600m along Z) with one corner at 200..300m. */
export function syntheticLap(lateralOffsetM: number, brakeShiftM: number): SemanticTelemetrySample[] {
  ...
  for (let i = 0; i < FRAMES_PER_LAP; i++) {
    ...
    samples.push({
      ...
      values: {
        ...
        "motion.position-x": inCorner ? lateralOffsetM : 0,
...
export function metadataArm(lapTimes: number[], labelPrefix = "arm"): { label: string; laps: ArmLap[] } {
  const laps: ArmLap[] = lapTimes.map((lapTime) => {
    ...
    return { lap, semanticSamples: null };
  });
...
export function telemetryArm(specs: { lateral: number; brakeShift: number }[], label = "telemetry-arm"): ArmInput {
  const laps: ArmLap[] = specs.map((spec, i) => ({
    ...
    semanticSamples: syntheticLap(spec.lateral, spec.brakeShift),
  }));
...
export function buildStreamingArm(specs: LapSpec[], firstId = 1) {
  const frames = new Map<number, SemanticTelemetrySample[]>();
  ...
  specs.forEach((spec, i) => {
    ...
    laps.push({ lap, semanticSamples: rawFrameCount > 0 ? syntheticLap(spec.lateral, spec.brakeShift) : null });
  });
...
export function trackingLoader(frames: Map<number, SemanticTelemetrySample[]>) {
  const decoded: number[] = [];
  const loadSamples: SemanticSampleLoader = async (lapId) => {
    decoded.push(lapId);
    return frames.get(lapId) ?? null;
  };
  return { loadSamples, decoded };
}
