...
const CORNER_COLORS = ["#ef4444", "#f97316", "#eab308", "#22c55e", "#06b6d4", "#3b82f6", "#8b5cf6", "#ec4899", "#14b8a6", "#f43f5e"];
 ...
 */
export function generateSegmentSvg(rawOutline: Pt[], segments: NamedSegment[], sectors: { s1End: number; s2End: number } | null, title: string, outFile: string, gameId: GameId): void {
  ...
  if (!tryGetGame(gameId)) {
    throw new Error(`game "${gameId}" is not registered (call initGameAdapters() before rendering): ` + `cannot determine coordinate system for "${title}"`);
  }
