...
describe("aligned telemetry fidelity", () => {
  ...
  test("crops chart range to first and last track indices", () => {
    const base = set([0, 1, 2, 3, 4]);
    const cropped = cropAlignedLapSet(base, 1, 3);
    expect(trackFractionRange(cropped)).toEqual({ start: 0.25, end: 0.75 });
    expect(trackFractionRange(base)).toEqual({ start: 0, end: 1 });
  });
