...
const frame = (values: readonly CanonicalTelemetryScalar[], schemaId="s", states?: FrameStates, freshness?: FrameFreshness): LiveTelemetryFrameMessageV1 => ({type:"telemetry-frame",protocolVersion:1,schemaId,streamId:"x",sessionId:null,sequence:1,observedAt:{domain:"session",milliseconds:2},receivedAtMs:3,values,states,freshness,context:{}});
describe("live telemetry view",()=>{
 ...
 it("returns undefined for non-fresh values",()=>{const stale=buildLiveTelemetryView(schema,frame([4,[1,2,3,4],.5],"s",undefined,{1:"stale"}))!; expect(stale.motion.speedMps).toBe(4); expect(stale.tires.pressurePsi).toBeUndefined();});
