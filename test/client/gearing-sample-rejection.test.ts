...
describe("viewToGearingSample", () => {
  ...
  it("maps the frame's race-active semantic", () => {
    const view = baseView();
...
describe("resolver projection to gearing sample", () => {
  ...
  it("rejects the sample when race state is stale", () => {
    const view = buildLiveTelemetryView(schema, frame({ freshness: { 11: "stale" } }));
    expect(view?.race.isRaceOn).toBeUndefined();
    expect(viewToGearingSample(view!)).toBeNull();
  });
});
