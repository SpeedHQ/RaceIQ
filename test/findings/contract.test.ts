...
function finding(overrides: Partial<FindingRecord> = {}): FindingRecord {
  const record: FindingRecord = {
    ...
    type: "braking-lockup",
    category: "driving",
    scope: { kind: "lap", sessionId: "session-1", participantId: "driver-1", lapId: "lap-1" },
    status: "available",
    severity: "medium",
    confidence: "high",
    ...
    evidenceRefs: [{ kind: "telemetry-range", id: "range-10-20", startFrameIndex: 10, endFrameIndex: 20, channel: "brake", semanticIds: ["brake"] }],
    qualityRefs: [],
    limitations: [],
...
describe("finding contract validation", () => {
  test("accepts available finding with typed measurements and exact evidence", () => {
    ...
    expect(record.evidenceRefs[0]).toMatchObject({ id: "range-10-20", startFrameIndex: 10, endFrameIndex: 20, channel: "brake" });
  });
