...
function lapFinding(lapId: string, loss: number, recurring: boolean, overrides: Partial<FindingRecord> = {}): FindingRecord {
  const value: FindingRecord = {
    ...
    category: "pace",
    scope: { kind: "lap", sessionId: "session-1", participantId: "driver-1", cornerId: "turn-1", lapId },
    status: "available",
    severity: "medium",
    confidence: "medium",
    measurements: [
    ...
    evidenceRefs: [{ kind: "lap", id: lapId, lapId }, { kind: "telemetry-range", id: `range-${lapId}`, startFrameIndex: 10, endFrameIndex: 20, channel: "speed" }],
    qualityRefs: [],
    limitations: [],
...
const target = { kind: "corner", sessionId: "session-1", participantId: "driver-1", cornerId: "turn-1" } as const;
...
describe("finding aggregation", () => {
  ...
  test("refuses cross-session, participant, and supplied context", () => {
    const base = lapFinding("lap-1", 1, true);
    const session = lapFinding("lap-2", 2, true, { scope: { kind: "lap", sessionId: "session-2", participantId: "driver-1", cornerId: "turn-1", lapId: "lap-2" } });
    expect(aggregateFindings([base, session], { targetScope: target })).toMatchObject({ reason: "incompatible-scope" });
    const participant = lapFinding("lap-2", 2, true, { scope: { kind: "lap", sessionId: "session-1", participantId: "driver-2", cornerId: "turn-1", lapId: "lap-2" } });
