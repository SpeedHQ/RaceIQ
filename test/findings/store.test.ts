...
describe("structured findings store", () => {
  ...
  test("activation replaces whole scope without mixed active generations", async () => {
    ...
    const stale = await getFindingGeneration(first.receipt.generationId);
    expect(stale?.receipt.status).toBe("stale-rebuild-available");
    expect(stale?.findings.map((record) => record.evidenceRefs[0]?.id)).toEqual(["lap-old"]);
  });
