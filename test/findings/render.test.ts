...
function finding(type: string, status: FindingRecord["status"]): FindingRecord {
  const record: FindingRecord = {
    ...
    type,
    category: "pace",
    scope: { kind: "lap", sessionId: "session-1", lapId: `lap-${type}` },
    status,
    severity: "medium",
    confidence: "low",
    ...
    evidenceRefs: [{ kind: "telemetry-range", id: `range-${type}`, startFrameIndex: 12, endFrameIndex: 34, startTimestampMs: 1200, endTimestampMs: 3400, channel: "speed" }],
    qualityRefs: [],
...
describe("deterministic findings report", () => {
  ...
  test("renders exact status, reason, confidence, measurements, units, samples, and evidence ranges", () => {
    ...
    expect(report).toContain("frames=12–34; timestamps-ms=1200–3400; channel=speed");
  });
