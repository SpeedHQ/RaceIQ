...
describe("adaptComparisonToFindings", () => {
  test("uses each corner range for A/B evidence and measurement sample counts", () => {
    ...
    expect(ranges[0]).toEqual([
      expect.objectContaining({ sessionId: "7", lapId: "41", startFrameIndex: 10, endFrameIndex: 12 }),
      expect.objectContaining({ sessionId: "9", lapId: "52", startFrameIndex: 30, endFrameIndex: 32 }),
    ]);
    expect(ranges[1]).toEqual([
      expect.objectContaining({ sessionId: "7", lapId: "41", startFrameIndex: 20, endFrameIndex: 24 }),
      expect.objectContaining({ sessionId: "9", lapId: "52", startFrameIndex: 40, endFrameIndex: 44 }),
    ]);
