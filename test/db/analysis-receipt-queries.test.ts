...
import { ANALYSIS_ARTIFACT_SET_IDENTITY_SEED, ANALYSIS_RECEIPT_SCHEMA_VERSION, type AnalysisProvenanceReceipt } from "../../shared/racing/provenance/contracts";
import { activateAnalysisGeneration, beginAnalysisGeneration, failAnalysisGeneration, deriveAnalysisArtifactSetId } from "../../server/db/analysis-receipt-queries";
import { db } from "../../server/db/index";
import { laps, raceEvents, sessions } from "../../server/db/schema";
import { currentAnalysisContract } from "../../server/analysis-provenance/current-contract";
import { auditPersistedSessionAnalysis, buildPersistedSessionAnalysisInventory } from "../../server/analysis-provenance/inventory";
import { createPersistedSessionAnalysisReceipt } from "../../server/analysis-provenance/receipt";
import { raceEventId } from "../../server/race-events/identity";
...
describe("analysis receipt persistence", () => {
  test("keeps v1 artifact-set IDs tied to frozen identity seed, not receipt schema evolution", () => {
    ...
    const legacyId = `analysis-set:${createHash("sha256")
      .update(canonicalJson(["analysis-receipt-v1", input.sessionId, input.participantId, input.artifactSetType]))
      .digest("hex")}`;

    const schemaCoupledId = `analysis-set:${createHash("sha256")
      .update(canonicalJson([simulatedFutureReceiptSchemaVersion, input.sessionId, input.participantId, input.artifactSetType]))
      .digest("hex")}`;
  ...
  test("rejects activation when pending or structured contract configuration identity differs", async () => {
    ...
    for (const [mutate, message] of mutations) {
      const { attempt, receipt } = await createAttemptWithReceipt(sessionId);
      await expect(activateAnalysisGeneration({ generationId: attempt.generationId, receipt: mutate(receipt) })).rejects.toThrow(message);
  ...
  test("audit detects ordered sector-time mutation", async () => {
    ...
    await db
      .update(laps)
      .set({ sectorTimes: [30, 28.5, 31.5] })
      .where(eq(laps.sessionId, sessionId))
      .run();

    expect(await auditPersistedSessionAnalysis(receipt)).toContainEqual(
  ...
  test("audit rejects extra and duplicate receipt output names", async () => {
    ...
    const mutations: AnalysisProvenanceReceipt[] = [
      {
        ...receipt,
        outputs: [...receipt.outputs, { ...firstOutput, name: "unexpected-output", artifactType: "findings" }],
      },
  ...
  test("keeps semantic output hashes independent of local session ids", async () => {
      ...
      await db
        ...
        .values({
          eventId: raceEventId({
            ...
            detectorId: "analysis-receipt-test",
            boundaryKey: "position:1",
            lifecycleId: null,
          }),
          eventType: "position_changed",
          ...
          payload: { previousPosition: 2, position: 1 },
          detectorId: "analysis-receipt-test",
          detectorVersion: "1",
    ...
    const first = await buildPersistedSessionAnalysisInventory(firstSessionId);
    const second = await buildPersistedSessionAnalysisInventory(secondSessionId);
    expect(first.outputs.find(({ name }) => name === "race_events")?.contentHash).toBe(second.outputs.find(({ name }) => name === "race_events")?.contentHash);
  });
});
