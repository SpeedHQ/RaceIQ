...
import { deleteSession, getSessions, insertSession, updateSessionQuality } from "../../server/db/session-queries";
...
import { LOCAL_PLAYER_EVIDENCE, type LapQualitySummary, type SourceChannelProfile } from "../../shared/racing/quality/contracts";
...
test("current session and lap queries round-trip quality and expose stale generations", async () => {
  const sessionId = await insertSession(990_260, 991_260, testGameId, "practice", TEST_VERSION_IDENTITY, "mine", "motec", sourceChannelProfile);
  sessionIds.push(sessionId);

  const packets = qualityPackets(50);
  const recordingAccumulator = new RecordingQualityAccumulator("motec", LOCAL_PLAYER_EVIDENCE, TEST_VERSION_IDENTITY);
  ...
  const persistedLapQuality = JSON.parse(JSON.stringify(finalizedLap.quality)) as typeof finalizedLap.quality;
  const persistedEligibility = JSON.parse(JSON.stringify(finalizedLap.eligibility)) as typeof finalizedLap.eligibility;
  ...
  const currentSession = (await getSessions(testGameId)).find((session) => session.id === sessionId);
  ...
  await db.update(sessions).set({ qualityGeneration: staleGeneration }).where(eq(sessions.id, sessionId)).run();
  await db.update(laps).set({ qualityGeneration: staleGeneration }).where(eq(laps.id, lapId)).run();

  const staleSession = (await getSessions(testGameId)).find((session) => session.id === sessionId);
...
test("missing session quality remains absent rather than stale", async () => {
  ...
  const session = (await getSessions(testGameId)).find((candidate) => candidate.id === sessionId);
...
test("session finalization promotes provisional lap quality", async () => {
  const packets = qualityPackets(80);
  const sessionId = await insertSession(990_264, 991_264, testGameId, "practice", TEST_VERSION_IDENTITY);
  sessionIds.push(sessionId);
  const provisionalQuality = summarize(packets);
  expect(provisionalQuality.provenance.outputGeneration).toBe("provisional");
  ...
  expect(stored?.qualityGeneration).toMatch(/^sha256:[0-9a-f]{64}$/);
  expect(stored?.quality?.provenance.outputGeneration).toBe(stored?.qualityGeneration);
  expect(stored?.qualityStale).toBe(false);
...
test("session and lap quality writes converge with recording gaps", async () => {
  ...
  const baseLap = finalizeLapQualityGeneration(summarize(lapPackets), finalizedRecording.provenance.sourceGeneration, { lapNumber: 1, rawByteOffset: null, rawFrameCount: 0 });

  async function persist(order: "session-first" | "lap-first") {
    const sessionId = await insertSession(990_261, 991_261, testGameId, "practice", TEST_VERSION_IDENTITY);
    sessionIds.push(sessionId);
  ...
  const gapFacts = sessionFirst?.quality?.facts.filter(({ code }) => code === "telemetry_gap_minor" || code === "telemetry_gap_major");
  expect(gapFacts?.map(({ code }) => code).sort()).toEqual(["telemetry_gap_major", "telemetry_gap_minor"]);
...
test("overlapping lap and recording gaps remain single measured facts", async () => {
  ...
  const finalizedRecording = finalizeRecordingQualityGeneration(recordingDraft);
  const baseLap = finalizeLapQualityGeneration(summarize(packets), finalizedRecording.provenance.sourceGeneration, { lapNumber: 1, rawByteOffset: null, rawFrameCount: 0 });
  const sessionId = await insertSession(990_262, 991_262, testGameId, "practice", TEST_VERSION_IDENTITY);
  sessionIds.push(sessionId);
  await updateSessionQuality(sessionId, recordingDraft);
  const lapId = await insertLap({
    sessionId,
  ...
  const stored = await getLapById(lapId);
  const gapFacts = stored?.quality?.facts.filter(({ code }) => code === "telemetry_gap_minor" || code === "telemetry_gap_major");
  expect(gapFacts?.map(({ code }) => code).sort()).toEqual(["telemetry_gap_major", "telemetry_gap_minor"]);
...
test("session quality updates do not promote legacy placeholder laps", async () => {
  const packets = qualityPackets(50);
  const sessionId = await insertSession(990_263, 991_263, testGameId, "practice", TEST_VERSION_IDENTITY);
  sessionIds.push(sessionId);
