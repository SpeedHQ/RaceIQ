...
} from "../../server/lap-analysis/quality-rebuild";
...
describe("quality rebuild detector identity", () => {
  ...
  ): Promise<{ id: number; quality: RecordingQualitySummary }> {
  ...
  function rawCapture(name: string): string {
    const path = join(testDir, name);
    writeFileSync(path, Buffer.from("capture"));
    return path;
  }
  ...
  test("rebuilds policy-only staleness without changing replay measurements", async () => {
    ...
    expect(replayMeasurements(row!.recordingQuality!)).toEqual(
      replayMeasurements(quality),
    );
  });
