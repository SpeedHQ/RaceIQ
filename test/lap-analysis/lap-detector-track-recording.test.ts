...
function packet(scenario: LapScenario, index: number, overrides: Partial<TelemetryPacket> = {}): TelemetryPacket {
  return {
    ...
    DistanceTraveled: 100 + index * 100,
    CarOrdinal: 42,
...
describe("LapDetector normal-pace geometry recording gates", () => {
  test("persists and emits a structurally valid iRacing pit lap without seeding outline or curb geometry", async () => {
    ...
    try {
      ...
      expect(result.completed[0].isValid).toBe(true);
      expect(result.completed[0].eligibility["normal-pace"].status).toBe("ineligible");
