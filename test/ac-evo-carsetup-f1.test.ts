...
describe("ac-evo carsetup parser (F1 / Ferrari SF25)", () => {
  ...
  it("front-left corner matches in-game values", () => {
    ...
    expect(fl["Slow bump? (#1)"]).toBe("12");
    expect(fl["Slow rebound? (#3)"]).toBe("7");
  ...
  it("mechanical & brakes match in-game values", () => {
    ...
    expect(mech["Brake bias"]).toBe("59.9% front");
    expect(mech["Diff preload"]).toBe("75 Nm");
    ...
    expect(mech["Front ARB stiffness"]).toBe("66 kN/m");
    expect(mech["Rear ARB stiffness"]).toBe("26 kN/m");
  });
  ...
  it("electronics match in-game values (ERS fields intentionally unmapped)", () => {
    ...
    expect(elec["Telemetry laps? (#5)"]).toBe("15");
    // ERS fields surface only as unlabeled generic rows until mapped.
    expect(elec["ERS deployment map"]).toBeUndefined();
    expect(elec["#8"]).toBe("3");
