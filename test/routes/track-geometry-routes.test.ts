...
import { loadTrackGeometryForGame, saveTrackMetadata } from "../../shared/racing/tracks/storage/meta";
import { loadTrackRegistrySource, readTrackRegistrySourceFiles, resolveTrackRegistryLocations } from "../../shared/racing/tracks/registry/source";
...
const TRACK_SECTOR_FIXTURE = (() => {
  ...
  for (const assignment of source.configurations.assignments) {
    ...
    const geometry = source.geometry.geometry.find((entry) => entry.factsSlug === factsSlug && entry.gameId === assignment.gameId);
...
async function setTrackSectors(s1End: number, s2End: number): Promise<void> {
  const response = await trackSectorBoundaryRoutes.request(`/api/track-sector-boundaries/${TRACK_SECTOR_FIXTURE.trackOrdinal}?gameId=${TRACK_SECTOR_FIXTURE.gameId}`, {
    method: "PUT",
...
describe("optional track geometry routes", () => {
  test("returns null instead of an error when curb data is unavailable", async () => {
    const response = await trackGeometryRoutes.request("/api/track-curbs/999999?gameId=iracing");
...
describe("track sector boundary routes", () => {
  test("requires validated gameId on GET", async () => {
    const response = await trackSectorBoundaryRoutes.request("/api/track-sector-boundaries/1");

    expect(response.status).toBe(400);
  });

  test("rejects invalid gameId on GET", async () => {
    const response = await trackSectorBoundaryRoutes.request("/api/track-sector-boundaries/1?gameId=unknown");
  ...
  test("reports editable RaceIQ timing boundaries for non-native games", async () => {
    const response = await trackSectorBoundaryRoutes.request("/api/track-sector-boundaries/1?gameId=fm-2023");
  ...
  test("persists sector and segment edits through isolated source and JSON read model", async () => {
    ...
    try {
      ...
      const geometryBefore = loadTrackRegistrySource().geometry.geometry.find((entry) => entry.factsSlug === TRACK_SECTOR_FIXTURE.factsSlug && entry.gameId === TRACK_SECTOR_FIXTURE.gameId);
      ...
      const geometryAfterSector = loadTrackRegistrySource().geometry.geometry.find((entry) => entry.factsSlug === TRACK_SECTOR_FIXTURE.factsSlug && entry.gameId === TRACK_SECTOR_FIXTURE.gameId);
      ...
      const sectorResponse = await trackSectorBoundaryRoutes.request(`/api/track-sector-boundaries/${TRACK_SECTOR_FIXTURE.trackOrdinal}?gameId=${TRACK_SECTOR_FIXTURE.gameId}`);
      ...
      const segmentResponseBefore = await trackSegmentRoutes.request(`/api/track-sectors/${TRACK_SECTOR_FIXTURE.trackOrdinal}?gameId=${TRACK_SECTOR_FIXTURE.gameId}`);
      ...
      const segmentPutResponse = await trackSegmentRoutes.request(`/api/tracks/${TRACK_SECTOR_FIXTURE.trackOrdinal}/segments?gameId=${TRACK_SECTOR_FIXTURE.gameId}`, {
        method: "PUT",
        headers: { "content-type": "application/json" },
      ...
      const geometryAfterSegments = loadTrackRegistrySource().geometry.geometry.find((entry) => entry.factsSlug === TRACK_SECTOR_FIXTURE.factsSlug && entry.gameId === TRACK_SECTOR_FIXTURE.gameId);
      ...
      const segmentResponseAfter = await trackSegmentRoutes.request(`/api/track-sectors/${TRACK_SECTOR_FIXTURE.trackOrdinal}?gameId=${TRACK_SECTOR_FIXTURE.gameId}`);
  ...
  test("serves generated read model only and persists explicit previews through canonical JSON", async () => {
    ...
    try {
      const missingResponse = await trackSegmentRoutes.request(`/api/track-sectors/${trackOrdinal}?gameId=${gameId}`);
      expect(missingResponse.status).toBe(200);
      expect(await missingResponse.json()).toEqual({ segments: [] });

      const generateResponse = await trackSegmentRoutes.request(`/api/tracks/${trackOrdinal}/segments/generate?gameId=${gameId}`, { method: "POST" });
      ...
      const assignmentAfter = sourceAfter.configurations.assignments.find((entry) => entry.gameId === gameId && entry.trackOrdinal === trackOrdinal);
      ...
      const geometryAfter = sourceAfter.geometry.geometry.find((entry) => entry.factsSlug === layoutAfter?.factsSlug && entry.gameId === gameId);
      ...
      const persistedResponse = await trackSegmentRoutes.request(`/api/track-sectors/${trackOrdinal}?gameId=${gameId}`);
