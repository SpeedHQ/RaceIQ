...
describe("session run routes", () => {
  test("parses intersecting session filters and default pagination", async () => {
    ...
    const response = await routes.request(
      `/api/sessions/42/runs?runKind=pace&participantId=car-1&driverId=driver-1&observedPhase=caution&timelineEpoch=2&status=incomplete&overlapsRunId=${encodeURIComponent(runId)}&minCompletedLaps=2&maxCompletedLaps=9&qualityOnly=true`,
    );
