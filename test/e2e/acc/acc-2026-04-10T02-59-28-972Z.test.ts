...
describe(recordingFile, () => {
  test("5 laps: outlap + 3 valid + incomplete tail", async () => {
    ...
    expect(laps.filter((l) => l.isValid).length).toBe(4);
    ...
    // Lap 1: joining outlap remains telemetry-valid but pace-ineligible.
    ...
    // lap-saved notifications preserve telemetry validity while pace excludes lap 1.
