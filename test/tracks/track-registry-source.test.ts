      ...
      facts: [
        {
          ...
          corners: [
            { number: 1, name: "Juncão", covers: [2, 3], direction: "left" },
...
describe("track registry venue metadata source", () => {
  ...
  test("canonicalizes file bytes and rejects legacy or noncanonical metadata shards", () => {
    withWorkspace((locations) => {
      ...
      writeFileSync(
        shardPath(locations, "venues/alpha/revisions/current/tracks/unexpected/metadata.json"),
        JSON.stringify({
          version: TRACK_REGISTRY_SOURCE_VERSION,
          id: "alpha/main",
          name: "Unexpected",
      ...
      writeFileSync(
        shardPath(locations, "venues/alpha/old/venue.json"),
        JSON.stringify({
          version: TRACK_REGISTRY_SOURCE_VERSION,
          id: "alpha/old",
          name: "Old",
  ...
  test("rejects layout removal while colocated assets remain", () => {
    withWorkspace((locations) => {
      ...
      expect(() =>
        updateTrackRegistrySource((draft) => {
          draft.configurations.layouts = draft.configurations.layouts.filter((layout) => layout.id !== "alpha/2010/legacy");
        }, locations),
      ).toThrow(/colocated asset remains/);
  ...
  test("rejects revision removal while revision assets remain", () => {
    withWorkspace((locations) => {
      ...
      expect(() =>
        updateTrackRegistrySource((draft) => {
          draft.configurations.venues = draft.configurations.venues.filter((venue) => venue.id !== "alpha/2010");
          draft.configurations.layouts = draft.configurations.layouts.filter((layout) => layout.id !== "alpha/2010/legacy");
        }, locations),
      ).toThrow(/colocated asset remains/);
