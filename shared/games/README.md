...
- CSV/json catalogs in `shared/games/*` are static data files and are expected to be loaded by server-side loader layers (`shared/car`, `shared/track`, `server/games/*`).

## Dependency direction
- Direction is core: `shared/games` defines contracts and adapters consumed by:
  - `shared/car` lookups
  - `shared/track` shared-name mapping
  - `shared/analysis` and UI analytics paths
...
- The Forza, F1, and ACC track extraction commands generate track geometry under `shared/tracks`; they do not regenerate game catalog CSVs.
