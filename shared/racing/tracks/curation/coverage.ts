...
export function renderDetailTables(rows: CoverageRow[] = curatedCoverage()): string {
  ...
  for (const r of rows) {
    out.push(
      `#### ${r.label}`,
      "",
      `${cell(r.curated, r.total)} curated · ${cell(r.metaVerified, r.total, r.metaStale)} facts-verified · ` +
        `${cell(r.segmentsVerified, r.total, r.segmentsStale)} geometry-verified`,
      "",
