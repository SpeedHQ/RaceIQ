...
/** Serialized track assignment schema version. */
...
/** Canonical slash-separated venue path, including optional revision hierarchy. */
...
/** One stable identity segment paired with display name. */
export const TrackIdentityNodeSchema = z.object({
  id: trackIdentityId,
  name: z.string().trim().min(1),
});
/** Implicit revision used by root-only venue paths. */
export const CURRENT_TRACK_REVISION = "current";

/** Stable root venue and revision derived from canonical venue path. */
...
/** Provenance proving game assignment was manually confirmed. */
...
/** Registry assignment linking one game ordinal to canonical venue and layout identity. */
...
/** Join canonical venue path and layout segment into layout ID. */
