...
export function createTrackGuideStore({ guidesDir, nested = false }: { guidesDir: string; nested?: boolean }): TrackGuideStore {
  ...
  const load = (slug: string, locale = "en"): TrackGuideFile | null => {
    ...
    if (guide && locale !== "en") {
      const overlayPath = nested ? resolve(dirname(path!), `guide.${locale}.json`) : resolve(overlayDir(guidesDir, locale), `${slug}.json`);
  ...
  const save = (guide: TrackGuideFile): TrackGuideFile => {
    ...
    list();
    const path = guidePaths.get(validated.id) ?? (nested ? nestedGuideTarget(guidesDir, validated.id) : resolve(guidesDir, `${validated.id}.json`));
