...
import type { EligibilityDecision, EligibilityDecisionSet, LapQualitySummary, QualityReasonCode } from "../quality/contracts";
...
export function selectEvaluationLaps<T extends EvaluableLap>(
  ...
  options: { requireSetupEligibility?: boolean } = {},
  ...
  for (const lap of laps) {
    ...
    if (lap.experimentExcludedSource === "manual" && lap.experimentExcluded) {
      ...
      const decisions = setupLapPolicyDecisions(lap);
      const policyFailures = [decisions.normalPace, decisions.cornerTrace].filter((decision) => !isEligibilityUsable(decision));
