...
try {
  ...
  for (const line of specsRaw.split(/\r?\n/).slice(1)) {
    ...
    fmCarSpecsCatalog.set(ordinal, {
      hp: Number.parseInt(fields[1], 10) || 0,
      torque: Number.parseInt(fields[2], 10) || 0,
      weightLbs: Number.parseInt(fields[3], 10) || 0,
      weightKg: Number.parseInt(fields[4], 10) || 0,
      displacement: Number.parseFloat(fields[5]) || 0,
      engine: fields[6] ?? "",
      drivetrain: fields[7] ?? "",
      gears: Number.parseInt(fields[8], 10) || 0,
      aspiration: fields[9] ?? "",
      frontWeightPct: Number.parseInt(fields[10], 10) || 0,
      pi: Number.parseInt(fields[11], 10) || 0,
      ...
      price: Number.parseInt(fields[16], 10) || 0,
      division: fields[17] ?? "",
