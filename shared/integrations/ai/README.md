...
- `language.ts` depends on `shared/platform/i18n/locales.ts`.
- Consumers import these leaves directly:
  - Server prompts: `shared/integrations/ai/language`, `shared/integrations/ai/prompt-snippets`
  - Client UI helpers: `shared/integrations/ai/context-window`
...
- Prefer leaf imports, e.g. `import { ADJUSTMENT_FORMAT_PROMPT } from "@shared/integrations/ai/prompt-snippets"`.
