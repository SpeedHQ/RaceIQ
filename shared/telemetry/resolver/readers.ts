...
export function trustedNativeExecutor(variable: TelemetryVariableDefinition, mapping: Exclude<Mapping, { kind: "unavailable" }>): Reader | undefined {
  ...
  if (variable.id === "tire.temperature.average" && nativeUnit === "°f") {
    if (Array.isArray(mapping.sources)) return undefined;
    const keyedSources = mapping.sources as Record<string, readonly string[]>;
    const ordering = variable.ordering ?? Object.keys(keyedSources);
    const rawValues = new Array<unknown>(ordering.length);
    const canonicalValues = new Array<number>(ordering.length);
    const observationKey = sourcePaths.join("|");
    return (frame, context) => {
      ...
      for (let index = 0; index < ordering.length; index += 1) {
        ...
        for (const source of sourcesForOrderingKey(keyedSources, ordering[index]) ?? []) {
          ...
          canonicalValues[index] = ((value - 32) * 5) / 9;
