...
export function sources(mapping: Exclude<Mapping, { kind: "unavailable" }>): readonly string[] {
...
export function packetField(frame: NativeObject, field: keyof TelemetryPacket): unknown {
  return frame[field] ?? (frame.packet !== null && typeof frame.packet === "object" ? (frame.packet as NativeObject)[field] : undefined);
}
function cardinalityAccepts(count: number, cardinality: TelemetryVariableDefinition["cardinality"]): boolean {
  return cardinality.kind === "scalar"
    ? count === 1
    : cardinality.kind === "fixed"
      ? count === cardinality.count
      : count >= cardinality.min && (cardinality.max === undefined || count <= cardinality.max);
}

function primitiveAccepts(value: unknown, valueType: Exclude<TelemetryVariableDefinition["valueType"], "structured">, enumDomain?: readonly string[]): boolean {
  if (valueType === "number") {
    return typeof value === "number" && Number.isFinite(value);
  }
  if (valueType === "boolean") return typeof value === "boolean";
  if (valueType === "string") return typeof value === "string";
  return typeof value === "string" && (enumDomain === undefined || enumDomain.includes(value));
}

function indexedLength(value: unknown): number | undefined {
  if (Array.isArray(value)) return value.length;
  if (ArrayBuffer.isView(value) && !(value instanceof DataView) && "length" in value && typeof value.length === "number") {
    return value.length;
  }
  return undefined;
}
...
function indexedValue(value: unknown, index: number): unknown {
  return value !== null && typeof value === "object" ? Reflect.get(value, String(index)) : undefined;
}

function structuredValueAccepts(variable: TelemetryVariableDefinition, input: unknown): boolean {
  ...
  const validate = (value: unknown, depth: number): boolean => {
    if (depth < schema.indices.length) {
      ...
      if (length === undefined || !cardinalityAccepts(length, schema.indices[depth].cardinality)) {
        return false;
      }
    ...
    return schema.fields.every((field) => primitiveAccepts(readPath(value, [field.id]), field.valueType, field.enumDomain));
  };

  return validate(input, 0);
}

function canonicalEnum(input: unknown, domain?: readonly string[]): string | typeof INVALID_VALUE {
  const value = typeof input === "string" ? input : typeof input === "number" && Number.isFinite(input) ? String(input) : undefined;
  return value !== undefined && (domain === undefined || domain.includes(value)) ? value : INVALID_VALUE;
}

export function canonicalValue(variable: TelemetryVariableDefinition, input: unknown): unknown | typeof INVALID_VALUE {
  const expectsCollection = variable.shape === "per-wheel" || variable.shape === "vector" || variable.shape === "array";
  if (expectsCollection && !Array.isArray(input)) return INVALID_VALUE;
  if (!expectsCollection && variable.shape !== "structured" && Array.isArray(input)) {
  ...
  if (Array.isArray(input)) {
    if (!cardinalityAccepts(input.length, variable.cardinality) || (variable.cardinality.kind !== "variable" && variable.ordering !== undefined && input.length !== variable.ordering.length)) {
      return INVALID_VALUE;
    }
  ...
  if (variable.valueType === "number") {
    return typeof input === "number" && Number.isFinite(input) ? input : INVALID_VALUE;
  }
  if (variable.valueType === "boolean") {
    if (typeof input === "boolean") return input;
    return typeof input === "number" && Number.isFinite(input) ? input !== 0 : INVALID_VALUE;
  }
