...
- `compileTelemetryResolver` and frame readers stay environment-neutral. Production projection lives in `server/telemetry/live-projector.ts`, `server/race-events/semantic-projector.ts`, and `server/telemetry/replay.ts`.
...
- Normal consumers depend on semantic IDs and resolver output. Native packets remain limited to parsers, resolver source readers, source-quality measurement, raw recording/replay decoding, canonical archive construction, canonical input hashing, and explicit development diagnostics.
