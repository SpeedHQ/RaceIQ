...
/**
 * Offline projection of iRacing track and track-asset responses. Its native
 * layout IDs are the same values emitted by the SDK as TrackID.
 */
const tracks = readFileSync(
  resolve(SHARED_DIR, "games/iracing/tracks.csv"),
  "utf-8",
)
  .split(/\r?\n/)
  .slice(1)
...
const tracksByOrdinal = new Map(
  tracks.map((track) => [track.ordinal, track]),
);
...
export function getIRacingTrack(
  ordinal: number,
): IRacingCatalogTrack | undefined {
...
export function getIRacingSharedTrackName(
  ordinal: number,
): string | undefined {
  return getIRacingTrack(ordinal)?.commonTrackName || undefined;
}

export function getIRacingTrackOrdinalByName(
  name: string,
): number | undefined {
  ...
  const byTrackName = tracks.filter(
    (track) => normalized(track.name) === needle,
  );
  return byTrackName.length === 1
    ? byTrackName[0].ordinal
    : undefined;
}
