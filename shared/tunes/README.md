...
- `bun run scripts/scrape-f1-setups.ts` updates F1 track identity, setup, guide, and provider metadata.
- `bun run scripts/scrape-f1-leaderboards.ts` updates F1Laps `_leaderboard.json` files and source timestamp.
- `bun run scripts/scrape-acc-setups.ts` updates ACC setup files and `acc/accsetups-com/_source.json`.
...
When adding a provider or game, add its generator first, keep stable game/track/car slugs, record source provenance, then generate the full directory. Do not add a barrel or runtime loader under `shared/tunes`.
