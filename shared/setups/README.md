...
- `shared/setups/*` may depend on shared primitive contracts such as `shared/games/ids.ts` and Zod.
...
## Add/extend safely
- New editable ACC/AC Evo field: add its dotted path, label, and arity in `schema.ts`; preserve simulator click values so setup JSON round-trips without normalization loss.
...
- `schema.ts` is source of truth for setup form coverage and dotted Kunos setup paths.
- `catalog/groups.ts`, `catalog/concepts.ts`, `catalog/file-source-mappings.ts`, and `catalog/parser-source-mappings.ts` are source inputs for semantic setup coverage in telemetry catalog.
...
```ts
import { getSchemaForGame, getByPath, setByPath } from "@shared/setups/schema";
import { AccSetupJsonSchema, setupFileFormat } from "@shared/setups/file-formats";
