...
`shared/track/` is executable TypeScript. Plural `shared/tracks/` is bundled CSV/JSON data; code in this directory consumes or produces those assets.
...
- `recording/`
- `curation/`
- `guide/` — contracts and loaders for static data in `shared/tracks/guides/`.

## Data split and join contract
- `shared/tracks/meta/<slug>.json`: physical roster only (turn numbers, names, groups, straights).
- `shared/tracks/<gameId>/<slug>-segments.json`: geometry only (fraction ranges per segment key).
...
## Add/extend safely
- Add/modify facts for a layout in `shared/tracks/meta/<slug>.json` and keep turn numbering complete and ordered.
- Add/refresh one-game geometry in `shared/tracks/<gameId>/<slug>-segments.json` via generation.
...
- Import explicit leaves (for example, `shared/track/storage/meta` or `shared/track/curation/generate`); this directory has no barrel contract.

## Verification files
- `shared/track/curation/verified.ts` records human sign-off hashes in `shared/tracks/verified.json`.
- `shared/track/curation/coverage.ts` renders curation coverage.
