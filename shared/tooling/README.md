...
`CHANGELOG.md` is source of truth. `scripts/generate-release-note.ts` generates release artifacts; `scripts/check-changelog.ts` runs validation. Keep these leaves pure string transformations with no filesystem, process, or Git access.
