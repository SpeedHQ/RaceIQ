 ...
 *
 * Mirrors shared/track-data.ts: same SHARED_DIR resolution (so the compiled
 * binary reads them from data/ next to the exe), same permanent cache with
 * negatives cached. The prose in these files is the only user-visible English
 * left in the AI prompt path that a translator can't reach from code, which is
 * why it is data rather than a TS literal.
 *
...
import type { TrackGuideCornerFile, TrackGuideFile } from "./track-guide-types";
 ...
 * shared/track-segment-align.ts — this repo validates committed track data with
