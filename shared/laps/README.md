...
`shared/telemetry` + `shared/sessions` + `shared/math` + `shared/lap-analysis/physics` -> `shared/laps` -> client/server review consumers
...
- Import explicit leaf modules such as `shared/laps/review-selection`; do not add a barrel.
