...
- `LOCALES` flows to:
  - `shared/ai/language.ts` for `aiLanguageInstruction`
...
- Locale source-of-truth for in-app locale membership is `shared/i18n/locales.ts`.
- Translation payloads are `client/messages/*.json`.
- Update sequence for adding a locale:
  1. Add locale entry in `shared/i18n/locales.ts`.
...
- Import locale data directly from `shared/i18n/locales`.
