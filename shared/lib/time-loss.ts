...
import { allWheelStates } from "./vehicle-physics";
 ...
 */
export function frameDt(telemetry: TelemetryPacket[]): number[] {
  const n = telemetry.length;
  const dt = new Array<number>(n).fill(1 / 60);
  if (n < 2) return dt;
  for (let i = 0; i < n - 1; i++) {
    const ms = telemetry[i + 1].TimestampMS - telemetry[i].TimestampMS;
