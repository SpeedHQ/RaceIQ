 ...
 *
 * Usage:
 *   bun run scripts/games/ac-evo/extract-cars.ts [kspkg]        # full car list from content.kspkg, appends new rows
 *   bun run scripts/games/ac-evo/extract-cars.ts --recordings   # scan .bin recordings instead
 *
...
import { runSetupRangesExtraction } from "./extract-setup-ranges";

const REPO_ROOT = resolve(import.meta.dir, "../../..");
const RECORDINGS_DIR = resolve(REPO_ROOT, "test", "artifacts", "sessions");
const CSV_PATH = resolve(REPO_ROOT, "shared", "games", "ac-evo", "cars.csv");
const V2_HEADER = 16;
...
function fromGame(explicitPath: string | undefined): void {
  ...
  let records: ReturnType<typeof parseCarsTable>;
  try {
