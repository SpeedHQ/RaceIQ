...
| `bun scripts/quality/bench-compare.ts <baseline.json> <current.json> [--threshold=5] [--p99-threshold=5] [--include=<prefix>] [--fail-on-regression]` | Compare Mitata median, p99, and allocation results with metric-specific tolerances and optional benchmark-prefix enforcement.  |
