...
export async function buildTelemetryCatalog(): Promise<BuiltTelemetryCatalog> {
  ...
  for (const gameId of GAME_IDS) {
    inventories[gameId].sort((a, b) => a.path.localeCompare(b.path, "en"));
  }
  ...
  const catalogWithoutHash: Omit<BuiltTelemetryCatalog, "metadata"> & {
    ...
    groups: [...groups.values()].sort((a, b) => a.id.localeCompare(b.id, "en")),
    variables: [...variables.values()].sort((a, b) => a.id.localeCompare(b.id, "en")),
    sources: inventories,
    coverage: {
