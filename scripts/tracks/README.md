...
Track curation and coverage commands. Authoring scripts update canonical venue/revision/layout metadata before regenerating compact JSON read-model and report artifacts; guide JSON and root TUMFTM boundary data remain separate outputs. Game-specific extractors stay under game domains.

## Commands

| Command | Purpose | Main output |
|---|---|---|
| `bun run tracks:registry` | Canonicalize registry source, rebuild artifacts, and validate bundled iRacing SVG layers | `shared/data/tracks/registry.json`, `shared/data/tracks/registry-report.json` |
...
- Authoring commands resolve source paths through shared configuration helpers, mutate metadata, then refresh read model/report. Generation never stamps verification; only human `--verify` does.
- Runtime loads generated JSON into memory and excludes source manifests/report/hints. Revision imagery, layout game geometry/guides, root shared ACC/TUMFTM geometry, and all four iRacing SVG layers ship.
