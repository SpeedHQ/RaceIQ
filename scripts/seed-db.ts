...
async function insertDemoRows(profileId: number, importedLapIds: number[]): Promise<void> {
  ...
  const tuneA = await db.insert(tunes).values({
    ...
    category: "road", trackOrdinal: fm?.trackOrdinal ?? 5, description: `Seeded demo tune (${SEED_MARKER})`,
  ...
  const tuneB = await db.insert(tunes).values({
    ...
    category: "road", trackOrdinal: fm?.trackOrdinal ?? 5, description: `Second-car tune (${SEED_MARKER})`,
  ...
  await db.insert(tuneAssignments).values({ gameId: "fm-2023", carOrdinal: fm?.carOrdinal ?? 3631, trackOrdinal: fm?.trackOrdinal ?? 5, tuneId: tuneA.id }).run();
  await db.insert(tuneAssignments).values({ gameId: "fm-2023", carOrdinal: secondCar, trackOrdinal: fm?.trackOrdinal ?? 5, tuneId: tuneB.id }).run();
  await db.update(laps).set({ profileId, tuneId: tuneA.id }).where(inArray(laps.id, importedLapIds)).run();

  const experiment = await db.insert(experiments).values({
    seq: 1, gameId: "fm-2023", name: `Silverstone demo experiment (${SEED_MARKER})`, carOrdinal: fm?.carOrdinal ?? 3631,
    trackOrdinal: fm?.trackOrdinal ?? 5, focus: "car", notes: SEED_MARKER,
