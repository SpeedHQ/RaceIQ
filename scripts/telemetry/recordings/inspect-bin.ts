...
if (!path) {
  console.error("Usage: bun scripts/telemetry/recordings/inspect-bin.ts <path-to-bin> [--game <gameId>] [--no-import]");
  process.exit(1);
}
...
const isGz = isGzip(raw);
const buf = gunzipIfNeeded(raw);
