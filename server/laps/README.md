...
- Archive layout stays `manifest.json` plus `<gameId>-<track>-session<id>.bin.gz` members. Version 2 exports carry member checksums and source-generation metadata; import requires valid checksums and exact manifest/member coverage. Version 1 or missing manifests use legacy source-fidelity behavior; any present malformed manifest is rejected. Member naming, sorted import order, compression, and the extra lap-trigger frame remain compatibility contracts.
