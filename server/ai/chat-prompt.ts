...
export function buildChatSystemPrompt(
  ...
  const normalizedSamples = samples.map((sample, index) => {
    const candidate = sample as SemanticTelemetrySample & { TimestampMS?: number };
    return candidate.values
      ? sample
      : {
          values: {},
          sequence: String(index),
          observedAtMs: candidate.TimestampMS ?? index,
        };
  });

  const exportText = generateExport(lap, normalizedSamples, unit, temperatureUnit);
  const cornerData = buildCornerData(normalizedSamples, corners, unit === "metric" ? "kmh" : "mph");

  const insights = analyzeLap(normalizedSamples, gameId, lap.quality);
  const semanticQuality = assessSemanticAnalysisInput(normalizedSamples, gameId, lap.lapTime);
