...
function statsLine(label: string, s: InputStats): string {
  return `${label}: thr avg ${pct(s.throttleAvg)}/max ${pct(s.throttleMax)}/full ${pct(s.fullThrottlePctDist)} | brk avg ${pct(s.brakeAvg)}/max ${pct(s.brakeMax)}/on ${pct(s.brakingPctDist)}/${s.brakeApplications}ev | str |avg| ${pct(s.steerAbsAvg)}/|max| ${pct(s.steerAbsMax)}/Δ${s.steeringSmoothness.toFixed(3)}`;
}
...
export function buildInputsComparePrompt(
  ...
  for (const seg of useSegs) {
    ...
    const sA = computeStatsRange(comparison.lapA.throttle, comparison.lapA.brake, comparison.lapA.steer, lo, hi);
    const sB = computeStatsRange(comparison.lapB.throttle, comparison.lapB.brake, comparison.lapB.steer, lo, hi);
...
Per-segment notes:
- "Fuel" lines show fuel level at the start and end of the segment plus consumption Δ. Use these to comment on whether one driver is burning more fuel through a section (e.g. heavier throttle, more wheel spin).
