...
export function computeLineSpreadTrace(laps: TelemetryPacket[][], lapIds: number[], corners: Corner[]): LineSpreadTrace | null {
  ...
  for (let i = 0; i < laps.length; i++) {
    ...
    const frac = span > 0 ? packets.map((packet) => round4(clamp01((packet.DistanceTraveled - base) / span))) : packets.map((_, index) => round4(index / Math.max(1, packets.length - 1)));
    lapLines.push({
      lapId: lapIds[i]!,
      x: x.map(round2),
      z: z.map(round2),
