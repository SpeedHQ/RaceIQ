...
export async function getQualityRebuildStatus(sessionId: number, currentDetectorIds: readonly string[] = []): Promise<QualityRebuildStatus> {
  ...
  const action: QualityRebuildAction =
    !session.recordingQuality || stale.schema || stale.detector ? (rawAvailable ? "reprocess" : "unavailable") : stale.policy || stale.configuration ? "rebuild_eligibility" : "current";
  return {
    sessionId,
    action,
    rawAvailable,
