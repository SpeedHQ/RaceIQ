...
export function parseAcEvoBuffers(physicsBuf: Buffer, graphicsBuf: Buffer, staticBuf: Buffer, cache: AcEvoParserCache): TelemetryPacket | null {
  ...
  const packet: TelemetryPacket = {
    ...
    acc,
    IsRaceOn: isRaceOn,
    TimestampMS: acEvoVersion === MOTEC_SYNTH_SOURCE_VERSION ? Math.round((Math.max(0, physPacketId) * 1_000) / MOTEC_SYNTH_HZ) : Date.now(),
