...
export function parseAcEvoBuffers(
  ...
  const packet: TelemetryPacket = {
    ...
    // v0.6 doesn't expose suspensionMaxTravel — leave as raw metres fraction unnormalised.
    // Use a nominal max of 0.1m (10cm) to get something in 0..1 range for display.
    NormSuspensionTravelFL: Math.min(1, suspFL / 0.1),
    NormSuspensionTravelFR: Math.min(1, suspFR / 0.1),
    NormSuspensionTravelRL: Math.min(1, suspRL / 0.1),
    NormSuspensionTravelRR: Math.min(1, suspRR / 0.1),
