...
export function convertAcEvoMotecToPackets(log: LdLog, beacons: number[], carTrack: MotecCarTrack): MotecConversionResult {
  ...
  for (let i = 0; i < prepared.frameCount; i++) {
    ...
    const packet = {
      ...
      CarOrdinal: carTrack.carOrdinal, TrackOrdinal: carTrack.trackOrdinal, CarClass: 0, CarPerformanceIndex: 0, DrivetrainType: 1, NumCylinders: 0, PositionX: prepared.path.x[i]!, PositionY: 0, PositionZ: prepared.path.z[i]!, Speed: speed, Power: 0, Torque: 0, WeatherType: 0, TrackTemp: 0, AirTemp: 0, RainPercent: 0,
