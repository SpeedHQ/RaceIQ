/**
 * Buffered Kunos memory reader with realistic update rates.
 *
...
/**
 * Shared-memory page layout, mapping names, and session detection for a
 * Kunos game. ACC and AC Evo use the same transport with distinct mappings
 * and layouts.
 */
...
export class BufferedKunosMemoryReader implements IRealtimeKunosMemoryReader {
  ...
  async stop(): Promise<void> {
    ...
    console.log(`[${this._opts.logPrefix} Buffered Reader] Stopped`);
  }
  ...
  private _tryConnect(): void {
    ...
    try {
      ...
      const physicsSize = this._opts.physicsSize;
      const graphicsSize = this._opts.graphicsSize;
      const staticSize = this._opts.staticSize;
      const FILE_MAP_READ = 0x0004;

      const physicsName = this._opts.physicsName;
      const graphicsName = this._opts.graphicsName;
      const staticName = this._opts.staticName;
      ...
      console.log(`[${this._opts.logPrefix} Buffered Reader] Connected to shared memory`);

      // Start polling at native rates
      this._startPolling();
    } catch (e) {
      console.error(`[${this._opts.logPrefix} Buffered Reader] Connection failed:`, e);
      this._disconnect();
    }
  }
  ...
  private _startPolling(): void {
    ...
    const sessionIdOffset = this._opts.sessionIdOffset;
        ...
        if (sessionIdOffset !== null) {
          ...
          if (!this._staticEverValid) {
            ...
            if (this._opts.staticValid?.(this._staticBuffer) ?? true) {
              ...
              console.log(`[${this._opts.logPrefix} Buffered Reader] Static page populated`);
            }
  ...
  private _readMapped(mapped: MappedFile): Buffer {
    ...
    if (duration > 5) {
      console.warn(`[${this._opts.logPrefix} Buffered Reader] Slow read: ${duration}ms for ${mapped.size} bytes`);
    }
