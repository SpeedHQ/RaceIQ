...
import { bundledGeometryPath, bundledSharedGeometryPath, getTrackAssetIdentity, isSharedAccGeometryAsset } from "../../../shared/racing/tracks/storage/assets";
...
export async function extractAccTracks(onProgress?: ProgressCallback): Promise<{ extracted: number }> {
  ...
  for (const dirName of trackDirs) {
    ...
    try {
      ...
      const geometryPath = identity && slug ? (isSharedAccGeometryAsset(identity) ? bundledSharedGeometryPath(identity, "acc", slug, "raceline") : bundledGeometryPath(identity, "raceline")) : null;
      const boundariesPath =
        identity && slug ? (isSharedAccGeometryAsset(identity) ? bundledSharedGeometryPath(identity, "acc", slug, "boundaries") : bundledGeometryPath(identity, "boundaries")) : null;
