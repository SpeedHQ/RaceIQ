...
import { bundledGeometryPath, bundledSharedAccGeometryPath, getTrackAssetIdentity, isSharedAccGeometryAsset } from "../../../shared/racing/tracks/storage/assets";
...
function validateWidths(widths: WidthRecord[]): boolean {
  ...
  return outliers / widths.length <= 0.1;
}
 ...
 */
function computeEdges(nodes: FastlaneNode[], widths: WidthRecord[], validWidths: boolean): { leftEdge: EdgePoint[]; rightEdge: EdgePoint[] } {
...
export async function extractAccTracks(onProgress?: ProgressCallback): Promise<{ extracted: number }> {
  ...
  const trackDirs = readdirSync(cacheDir).filter((d) => {
    try {
      return statSync(join(cacheDir, d)).isDirectory();
    } catch {
      return false;
    }
  });
  ...
  for (const dirName of trackDirs) {
    ...
    try {
      ...
      const geometryPath = identity && slug ? (isSharedAccGeometryAsset(identity) ? bundledSharedAccGeometryPath(identity, slug, "raceline") : bundledGeometryPath(identity, "raceline")) : null;
      const boundariesPath = identity && slug ? (isSharedAccGeometryAsset(identity) ? bundledSharedAccGeometryPath(identity, slug, "boundaries") : bundledGeometryPath(identity, "boundaries")) : null;
      ...
      console.log(`[ACC] ${dirName} (ordinal ${ordinal}, name "${slug}") — ${nodes.length} nodes${validWidths ? "" : " [default width]"} → OK`);
      extracted++;
