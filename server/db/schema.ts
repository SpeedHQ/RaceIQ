import { sqliteTable, text, integer, real, blob, index, unique, primaryKey } from "drizzle-orm/sqlite-core";
...
export const pitEvents = sqliteTable(
  ...
  (table) => [unique().on(table.resultId, table.sequence), index("idx_pit_events_result").on(table.resultId, table.sequence)],
);
...
export const georeferenceReferences = sqliteTable(
  "georeference_references",
  {
    ...
    referencePath: text("reference_path", { mode: "json" }).$type<readonly { latitudeDeg: number; longitudeDeg: number; altitudeM: number }[]>().notNull(),
  ...
  (table) => [unique().on(table.canonicalSlug, table.sourceIdentity), index("idx_georef_references_slug").on(table.canonicalSlug)],
);
...
export const georeferenceTransforms = sqliteTable(
  ...
  (table) => [
    unique().on(table.canonicalSlug, table.targetGameId, table.targetTrackOrdinal, table.referenceVersion),
