...
import {
  getIRacingSvgTrackMap,
  type IRacingMapLabel,
} from "../games/iracing/track-map";
...
interface ResolvedTrackOutline {
 ...
 */
async function resolveTrackOutline(
  ordinal: number,
  gameId: string,
  ...
  if (gameId === "iracing") {
    if (sharedName) {
      ...
      const labelledSegments = loadLabelledSegments(
        sharedName,
        "iracing",
      );
...
async function resolveTrackSegments(
  ...
  const resolved = gameId
    ? await resolveTrackOutline(ordinal, gameId)
    : null;
