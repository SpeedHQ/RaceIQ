...
export const trackCatalogInfoRoutes = new Hono()
  ...
  .get("/api/tracks/:ordinal", zValidator("param", OrdinalParamSchema), (c) => {
  ...
  .get("/api/tracks", async (c) => {
    ...
    if (gameId === "f1-2025") {
      ...
      const tracks = Array.from(f1Tracks.entries()).map(([id, info]) => {
        const hasBundled = !!getTrackOutlineByOrdinal(id, "f1-2025");
        return {
          ordinal: id,
          name: info.name,
    ...
    if (gameId === "acc") {
      ...
      const tracks = Array.from(accTracks.entries()).map(([id, info]) => {
        const hasBundled = !!getTrackOutlineByOrdinal(id, "acc");
        return {
          ordinal: id,
          name: info.name,
          location: "",
    ...
    if (gameId === "ac-evo") {
      ...
      const tracks = Array.from(acEvoTracks.entries()).map(([id, info]) => {
        const hasBundled = !!getTrackOutlineByOrdinal(id, "ac-evo");
        return {
          ordinal: id,
          name: info.name,
          location: "",
    ...
    if (gameId === "iracing") {
      ...
      const catalogIds = new Set(catalogTracks.map((track) => track.ordinal));
      const catalogEntries = catalogTracks.map((info) => {
        const hasShared = !!(loadLegacyOutlineByOrdinal(info.ordinal, "iracing") && info.commonTrackName && loadLabelledSegments(info.commonTrackName, "iracing").length > 0);
        const hasGenerated = sharedHasRecordedOutline(info.ordinal, "iracing");
        const hasOfficialSvg = !!info.mapUrl;
        const hasOutline = hasShared || hasOfficialSvg || hasGenerated;
        return {
          ...
          outlineSource: hasShared ? "shared" : hasOfficialSvg ? "official-svg" : hasGenerated ? "generated" : null,
          commonTrackName: info.commonTrackName || null,
