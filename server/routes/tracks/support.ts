...
export async function resolveTrackOutline(ordinal: number, gameId: string): Promise<ResolvedTrackOutline | null> {
  ...
  if (gameId === "iracing") {
    ...
    for (const lap of laps) {
      ...
      if (!saved || saved.gameId !== "iracing" || saved.trackOrdinal !== ordinal || saved.ownership !== "mine" || !isTimedLapEligibilityUsable(saved, "corner-trace") || saved.telemetry.length < 50) {
        continue;
      }
