...
export const trackLeaderboardRoutes = new Hono()
  ...
  .get("/api/tracks/:trackOrdinal/leaderboard", zValidator("param", TrackOrdinalParamSchema), async (c) => {
    ...
    const trackLaps = (await getLaps(gameId)).filter((lap) => lap.trackOrdinal === trackOrdinal && isTimedLapEligibilityUsable(lap) && lap.gameId === gameId);
  ...
  .get("/api/tracks/:trackOrdinal/all-laps", zValidator("param", TrackOrdinalParamSchema), async (c) => {
    const { trackOrdinal } = c.req.valid("param");
    const gameId = c.req.query("gameId") as GameId | undefined;
