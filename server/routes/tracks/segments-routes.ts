...
import { loadTrackFacts, loadTrackGeometryForGame, loadTrackSectorsFor, saveTrackGeometry, saveTrackMetadata } from "../../../shared/racing/tracks/storage/meta";
...
import { generateTrackSegments, computeOutlineLength, getSharedTrackName, requireGameId, resolveTrackSegments, TrackOrdinalParamSchema } from "./support";
...
export const trackCornerRoutes = new Hono()
  ...
  .get("/api/tracks/:trackOrdinal/corners", zValidator("param", TrackOrdinalParamSchema), async (c) => {
    ...
    if (dbCorners.length > 0) {
      const outline = getTrackOutlineByOrdinal(trackOrdinal, cornersGameId);
  ...
  .put("/api/tracks/:trackOrdinal/corners", zValidator("param", TrackOrdinalParamSchema), zValidator("query", RequiredGameIdQuerySchema), async (c) => {
    ...
    for (const corner of body) {
      if (typeof corner.index !== "number" || typeof corner.label !== "string" || typeof corner.distanceStart !== "number" || typeof corner.distanceEnd !== "number") {
        return c.json({ error: "Each corner must have index, label, distanceStart, distanceEnd" }, 400);
      }
      if (corner.distanceEnd <= corner.distanceStart) {
        return c.json({ error: `Corner ${corner.label}: distanceEnd must be > distanceStart` }, 400);
      }
    }
...
export const trackSectorBoundaryRoutes = new Hono()
  ...
  .put("/api/track-sector-boundaries/:ordinal", zValidator("param", OrdinalParamSchema), zValidator("query", RequiredGameIdQuerySchema), async (c) => {
    if (!IS_DEV && !IS_E2E) return c.json({ error: "Not available in production" }, 403);
    const { ordinal } = c.req.valid("param");
    const { gameId } = c.req.valid("query");
...
export const trackSegmentRoutes = new Hono()
  ...
  .put("/api/tracks/:trackOrdinal/segments", zValidator("param", TrackOrdinalParamSchema), zValidator("query", RequiredGameIdQuerySchema), async (c) => {
    if (!IS_DEV && !IS_E2E) return c.json({ error: "Not available in production" }, 403);
    const { trackOrdinal } = c.req.valid("param");
    const { gameId } = c.req.valid("query");
  ...
  .post("/api/tracks/:trackOrdinal/segments/generate", zValidator("param", TrackOrdinalParamSchema), zValidator("query", RequiredGameIdQuerySchema), async (c) => {
    if (!IS_DEV && !IS_E2E) return c.json({ error: "Not available in production" }, 403);
    const { trackOrdinal } = c.req.valid("param");
    const { gameId } = c.req.valid("query");
  ...
  .get("/api/track-sectors/:ordinal", zValidator("param", OrdinalParamSchema), zValidator("query", GameIdQuerySchema), async (c) => {
    const { ordinal } = c.req.valid("param");
  ...
  .get("/api/track-guide/:ordinal", zValidator("param", OrdinalParamSchema), zValidator("query", GameIdQuerySchema), async (c) => {
    const { ordinal } = c.req.valid("param");
    const gameId = c.req.query("gameId");
    const slug = getSharedTrackName(ordinal, gameId);
