...
import { extractCurbSegmentsFromSemanticSamples, getTrackCurbs, recordCurbData } from "../../../shared/racing/tracks/recording/curbs";
...
import { getSharedTrackName, requireGameId, TRACK_CURB_SEMANTIC_IDS, TRACK_POSITION_SEMANTIC_IDS } from "./support";
...
export const trackCalibrationRoutes = new Hono()
  ...
  .post("/api/track-calibration/:ordinal/from-lap", zValidator("param", OrdinalParamSchema), async (c) => {
    ...
    const lapData = await getLapMetaById(body.lapId);
    if (!lapData || lapData.gameId !== gameId) return c.json({ error: "Lap not found" }, 404);
    if (lapData.trackOrdinal !== ordinal) return c.json({ error: "Lap is not from this track" }, 400);

    const replay = await queryLapTelemetryBySemanticId(lapData.id, TRACK_POSITION_SEMANTIC_IDS);
    ...
    if (positions.length < 50) return c.json({ error: "Lap has insufficient position telemetry" }, 400);
...
export const trackGeometryRoutes = new Hono()
  ...
  .post("/api/track-curbs/:ordinal/extract", zValidator("param", OrdinalParamSchema), async (c) => {
    const { ordinal } = c.req.valid("param");
    const curbGameId = requireGameId(c);
    const trackLaps = (await getLaps(curbGameId)).filter((lap) => lap.trackOrdinal === ordinal && lap.lapTime > 0);
    ...
    for (const lap of trackLaps) {
      const lapData = await getLapMetaById(lap.id);
      if (!lapData || lapData.gameId !== curbGameId) continue;
      const replay = await queryLapTelemetryBySemanticId(lapData.id, TRACK_CURB_SEMANTIC_IDS);
      if (!replay) continue;
      const segments = extractCurbSegmentsFromSemanticSamples(semanticSamplesFromReplay(replay));
