...
import { loadLegacyBoundaryByOrdinal } from "../../../shared/racing/tracks/geometry/legacy";
...
import { requireGameId } from "./support";
...
export const trackGeometryRoutes = new Hono()
  ...
  .get("/api/track-boundaries/:ordinal", zValidator("param", OrdinalParamSchema), zValidator("query", GameIdQuerySchema), async (c) => {
    ...
    // Legacy boundaries only work for Forza, which has calibration transforms.
    // F1/ACC use different coordinate spaces, so baseline data would be misaligned.
    if (validGameId !== "fm-2023") return c.json(null);

    // Fall back to legacy boundaries, which need a coordinate transform.
    type LegacyBoundary = { leftEdge: { x: number; z: number }[]; rightEdge: { x: number; z: number }[]; pitLane: { x: number; z: number }[] | null };
    const boundaries: LegacyBoundary | null = loadLegacyBoundaryByOrdinal(ordinal, validGameId);
    if (!boundaries) return c.json(null);

    // If we have a recorded Forza-coords outline and a legacy baseline outline,
    ...
    // Transform legacy coordinates into Forza coordinates.
    ...
    // No transform available — return raw legacy coordinates.
    return c.json({
