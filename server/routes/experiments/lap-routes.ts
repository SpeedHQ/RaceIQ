...
export const experimentLapRoutes = new Hono()
  ...
  .get("/api/experiments/:id/importable-laps", zValidator("param", IdParamSchema), async (c) => {
    const { id } = c.req.valid("param");
    const session = await getExperiment(id);
    if (!session) return c.json({ error: "Tuning session not found" }, 404);

    const importable = await getImportableLapsForExperiment(session.gameId as GameId, session.carOrdinal ?? null, session.trackOrdinal ?? null);

    if (session.gameId !== "f1-2025") {
      return c.json(importable);
    }

    // F1 setup authority is persisted `carSetup`; older laps without it
    // intentionally remain unavailable rather than re-reading packet payloads.
    const enriched = importable.map((lap) => {
      const setup = resolveLapF1Setup(lap.carSetup ?? null);
      return {
        ...lap,
  ...
  .post("/api/experiments/:id/import-laps", zValidator("param", IdParamSchema), zValidator("json", ImportLapsSchema), async (c) => {
    const { id } = c.req.valid("param");
    const session = await getExperiment(id);
    if (!session) return c.json({ error: "Tuning session not found" }, 404);
    ...
    if (session.gameId !== "f1-2025") {
      ...
      const importedIds = await importLapsToExperiment(id, body.lapIds, body.experimentVersionId ?? null);
    ...
    const importableById = new Map(importable.map((lap) => [lap.id, lap]));
    for (const lapId of body.lapIds) {
      const setup = resolveLapF1Setup(importableById.get(lapId)?.carSetup);
    ...
    try {
      await saveChatMessages(tuneSessionThreadId(id), [
        ...
        {
          role: "assistant",
          markdown: `Imported ${importedIds.length} lap${importedIds.length === 1 ? "" : "s"}, sorted into ${distinctSetupCount} setup${distinctSetupCount === 1 ? "" : "s"}.`,
        },
      ]);
    } catch (err: any) {
      console.error("[tune] Failed to post import-laps note:", err?.message);
...
export const experimentLapAnalysisRoutes = new Hono()
  ...
  .get("/api/experiments/:id/lap-metrics", zValidator("param", IdParamSchema), async (c) => {
    const { id } = c.req.valid("param");
    const session = await getExperiment(id);
    if (!session) return c.json({ error: "Tuning session not found" }, 404);
    ...
    const missIds = sessionLaps.filter((l) => l.fuelPerLap == null && l.tyreWear == null).map((l) => l.id);
    const replayById = new Map(
      (await Promise.all(missIds.map(async (lapId) => [lapId, await queryLapTelemetryBySemanticId(lapId, CANONICAL_LAP_ANALYSIS_SEMANTIC_IDS)] as const))).map(([lapId, replay]) => [lapId, replay]),
    );
    ...
    for (const lapMeta of sessionLaps) {
      ...
      const replay = replayById.get(lapMeta.id);
      const samples = replay ? semanticSamplesFromReplay(replay) : [];
      const fuelPerLap = deriveFuelPerLap(samples);
      const tyreWear = deriveTyreWear(samples);
      if (replay) await setLapMetrics(lapMeta.id, fuelPerLap ?? null, tyreWear ?? null);
  ...
  .get("/api/experiments/:id/line-spread", zValidator("param", IdParamSchema), async (c) => {
    const { id } = c.req.valid("param");
    ...
    const loadedLaps = (
      await Promise.all(
        clean.map(async (meta) => {
          const replay = await queryLapTelemetryBySemanticId(meta.id, CANONICAL_LAP_ANALYSIS_SEMANTIC_IDS);
          return replay ? { meta, telemetry: semanticSamplesFromReplay(replay) } : null;
        }),
      )
    ).filter((lap): lap is { meta: LapMeta; telemetry: SemanticTelemetrySample[] } => lap !== null && lap.telemetry.length >= 30);
    ...
    const corners = await resolveSemanticLapCorners(session?.trackOrdinal, session?.gameId, fastest.telemetry);

    const trace = computeLineSpreadTrace(
      loadedLaps.map((lap) => lap.telemetry),
