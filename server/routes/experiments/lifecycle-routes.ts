...
export const experimentLifecycleRoutes = new Hono()
  .get("/api/experiments", zValidator("query", ExperimentQuerySchema), async (c) => {
  ...
  .post("/api/experiments", zValidator("json", CreateExperimentSchema), async (c) => {
  ...
  .patch("/api/experiments/:id/focus", zValidator("param", IdParamSchema), zValidator("json", SetFocusSchema), async (c) => {
  ...
  .get("/api/experiments/:id/focus-history", zValidator("param", IdParamSchema), async (c) => {
    const { id } = c.req.valid("param");
    const session = await getExperiment(id);
    if (!session) return c.json({ error: "Experiment not found" }, 404);
  ...
  .post("/api/experiments/:id/activate", zValidator("param", IdParamSchema), async (c) => {
    const { id } = c.req.valid("param");
    const session = await getExperiment(id);
  ...
  .post("/api/experiments/:id/deactivate", zValidator("param", IdParamSchema), async (c) => {
    const { id } = c.req.valid("param");
...
export const experimentDetailRoutes = new Hono()
  .get("/api/experiments/:id", zValidator("param", IdParamSchema), async (c) => {
    const { id } = c.req.valid("param");
  ...
  .patch("/api/experiments/:id", zValidator("param", IdParamSchema), zValidator("json", UpdateExperimentSchema), async (c) => {
    const { id } = c.req.valid("param");
    const body = c.req.valid("json");
