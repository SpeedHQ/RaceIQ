...
export const tuneChatRoutes = new Hono()
  .get("/api/laps/:id/issues", zValidator("param", IdParamSchema), zValidator("query", RequiredGameIdQuerySchema), async (c) => {
    ...
    if (samples.length < 30) return c.json([]);

    const corners = await resolveSemanticLapCorners(lap.trackOrdinal, lap.gameId, samples);
    const symptoms = telemetryToSymptoms(lap.gameId, samples, corners);
  ...
  .post("/api/live-analysis", zValidator("json", LiveAnalysisSchema), async (c) => {
  ...
  .get("/api/experiments/:id/chat", zValidator("param", IdParamSchema), async (c) => {
    const { id } = c.req.valid("param");
    try {
  ...
  .post("/api/experiments/:id/chat", zValidator("param", IdParamSchema), zValidator("json", TuneChatBodySchema), async (c) => {
    const { id } = c.req.valid("param");
    ...
    const turnContext = [sessionSystemPrompt, gatheredContext, extendedContext].filter(Boolean).join("\n\n");
  ...
  .delete("/api/experiments/:id/chat", zValidator("param", IdParamSchema), async (c) => {
    const { id } = c.req.valid("param");
    try {
