...
import { getActiveVerifiedCanonicalArchive, enqueueCanonicalArchiveJob, claimCanonicalArchiveJob, heartbeatCanonicalArchiveJob, completeCanonicalArchiveJob, failCanonicalArchiveJob, recoverExpiredCanonicalArchiveJobs } from "../db/canonical-archive-queries";
...
async function enqueueStableCaptureJobs(): Promise<void> {
  ...
  for (const row of rows) {
    ...
    const archive = await getActiveVerifiedCanonicalArchive(row.id);
    if (archive?.sourceContentHash === identity.contentHash) continue;
    await enqueueCanonicalArchiveJob({ sessionId: row.id, sourceContentHash: identity.contentHash });
  }
}
