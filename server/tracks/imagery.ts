...
/** Resolved private texture file plus modification timestamp used for HTTP caching. */
export interface LoadedTrackImageryTexture {
  path: string;
  modifiedAtMs: number;
}

/** Runtime imagery payload with public manifest data and server-only pack paths. */
...
/** Resolve authored venue imagery directory under shared track assets. */
...
/** Resolve game assignment's per-layout imagery manifest path. */
...
/** Load validated per-layout imagery layer selection, or null when assignment has none. */
...
/** Load validated venue imagery manifest, or null when venue has no imagery. */
...
/** Enumerate authored venue and layout imagery reachable from registry assignments. */
...
/** Resolve complete runtime imagery, verified pack metadata, and overlay texture files for one assignment. */
...
/** Infer supported overlay texture MIME type from authored file name. */
...
/** Derive stable user-image cache path without exposing raw track name. */
...
/** Return cache-busted HTTP URL for stored user base image, or null when absent. */
...
/** Decode, orient, bound, and normalize user base image to WebP before storage. */
