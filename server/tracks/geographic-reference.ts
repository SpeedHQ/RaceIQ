...
type GeographicIRacingCatalogTrack = IRacingCatalogTrack & { latitude: number; longitude: number };

export interface ResolvedTrackGeographicCatalogSource {
  track: GeographicIRacingCatalogTrack;
...
function hasGeographicLocation(track: IRacingCatalogTrack | undefined): track is GeographicIRacingCatalogTrack {
  const candidate = track as GeographicIRacingCatalogTrack | undefined;
  return (
    !!candidate &&
...
export function resolveTrackGeographicCatalogSource(gameId: GameId, trackOrdinal: number): ResolvedTrackGeographicCatalogSource | null {
  ...
  let venueReference: GeographicIRacingCatalogTrack | undefined;
...
export function alignTrackOutlineToReference(target: readonly TrackImageryPoint[], reference: readonly TrackImageryPoint[]): { points: TrackImageryPoint[]; rmseM: number } | null {
