...
/** SQLite schema version stored in imagery packs. */
export const TRACK_IMAGERY_PACK_SCHEMA_VERSION = 1 as const;
/** Only resolution tier currently supported by runtime tile routes. */
export const TRACK_IMAGERY_PACK_TIER = "hq" as const;
/** Encoded tile format required by imagery packs. */
export const TRACK_IMAGERY_PACK_FORMAT = "webp" as const;

/** Validated pack-wide dimensions, grid geometry, geographic bounds, and content identity. */
...
/** One encoded tile and its exact position within pack grid. */
...
/** Sync or async tile stream accepted by pack writer. */
export type TrackImageryPackTileSource = Iterable<TrackImageryPackTile> | AsyncIterable<TrackImageryPackTile>;

/** Cancellation and deadline controls checked throughout pack creation. */
...
/** Validate tiles, compute deterministic content hash, and atomically replace target SQLite pack. */
...
/** Open pack read-only and validate schema plus canonical metadata. */
...
/** Read and validate one HQ tile, returning null for coordinates outside declared grid. */
