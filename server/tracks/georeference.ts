...
import { applyAlignment, computeAlignment, trackAlignmentRmse, type TrackAlignment } from "../../shared/racing/tracks/geometry/points";
...
export function projectTrackImageryGeographicPositions(positions: readonly (GeographicPosition | null)[]): (TrackImageryGeographicPoint | null)[] {
  return positions.map((position) => (position ? { latitudeDeg: position.latitudeDeg, longitudeDeg: position.longitudeDeg } : null));
}
...
function nativePosition(packet: TelemetryPacket): GeographicPosition | null {
  const geo = packet.iracing as
    ...
    | undefined;
  if (!geo || !finite(geo.latitudeDeg) || !finite(geo.longitudeDeg) || !finite(geo.altitudeM)) return null;
  if (geo.latitudeDeg < -90 || geo.latitudeDeg > 90 || geo.longitudeDeg < -180 || geo.longitudeDeg > 180) return null;
  return {
...
function referenceVersion(path: readonly GeodeticReferencePoint[]): string {
  return createHash("sha256").update(JSON.stringify(path)).digest("hex").slice(0, 32);
}

function toEnu(point: GeodeticReferencePoint, origin: GeodeticReferencePoint): EastNorthPoint {
  const lat0 = (origin.latitudeDeg * Math.PI) / 180;
  return {
    east: (((point.longitudeDeg - origin.longitudeDeg) * Math.PI) / 180) * EARTH_RADIUS_M * Math.cos(lat0),
    north: (((point.latitudeDeg - origin.latitudeDeg) * Math.PI) / 180) * EARTH_RADIUS_M,
  };
}

function resampleByArc<T>(points: readonly T[], count: number, distance: (a: T, b: T) => number, interpolate: (a: T, b: T, fraction: number) => T): T[] {
  if (count <= 0 || points.length === 0) return [];
  if (points.length === 1) return Array.from({ length: count }, () => points[0]!);
  const cumulative = new Array<number>(points.length).fill(0);
  for (let index = 1; index < points.length; index++) {
    cumulative[index] = cumulative[index - 1]! + Math.max(0, distance(points[index - 1]!, points[index]!));
  }
  const total = cumulative[cumulative.length - 1]!;
  if (total <= 0) {
    return Array.from({ length: count }, (_, index) => {
      const position = count <= 1 ? 0 : (index * (points.length - 1)) / (count - 1);
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    let after = 1;
    while (after < cumulative.length - 1 && cumulative[after]! < target) after++;
    const before = after - 1;
    const span = cumulative[after]! - cumulative[before]!;
    return interpolate(points[before]!, points[after]!, span > 0 ? (target - cumulative[before]!) / span : 0);
  });
}

function arcFractions<T>(points: readonly T[], distance: (a: T, b: T) => number): number[] {
  const cumulative = new Array<number>(points.length).fill(0);
  for (let index = 1; index < points.length; index++) {
    cumulative[index] = cumulative[index - 1]! + Math.max(0, distance(points[index - 1]!, points[index]!));
  }
  const total = cumulative[cumulative.length - 1]!;
  return total > 0 ? cumulative.map((value) => value / total) : cumulative.map((_, index) => (points.length <= 1 ? 0 : index / (points.length - 1)));
}

function interpolateAtFraction<T>(points: readonly T[], fractions: readonly number[], fraction: number, interpolate: (a: T, b: T, fraction: number) => T): T {
  if (points.length === 1) return points[0]!;
  let after = 1;
  while (after < fractions.length - 1 && fractions[after]! < fraction) after++;
  const before = after - 1;
  const span = fractions[after]! - fractions[before]!;
  return interpolate(points[before]!, points[after]!, span > 0 ? (fraction - fractions[before]!) / span : 0);
}
...
function applyTransform(point: LocalPoint, transform: Transform): EastNorthPoint {
...
function geographyFromTransform(points: readonly LocalPoint[], referencePath: readonly GeodeticReferencePoint[], origin: GeodeticReferencePoint, transform: Transform): (GeographicPosition | null)[] {
  const localFractions = arcFractions(points, (a, b) => Math.hypot(b.x - a.x, b.z - a.z));
  ...
  const refs = localFractions.map((fraction) =>
    interpolateAtFraction(referencePath, referenceFractions, fraction, (a, b, f) => ({
  ...
  return points.map((point, index) => {
    ...
    return {
      latitudeDeg: origin.latitudeDeg + ((mapped.north / EARTH_RADIUS_M) * 180) / Math.PI,
      longitudeDeg: origin.longitudeDeg + ((mapped.east / (EARTH_RADIUS_M * cosLat)) * 180) / Math.PI,
      altitudeM: ref.altitudeM,
    };
  });
}

export async function assignLapGeoreference(input: { canonicalSlug?: string; gameId: GameId; trackOrdinal: number; packets: readonly TelemetryPacket[] }): Promise<GeoreferenceMetadata | null> {
  if (input.gameId === "iracing") {
    ...
    if (input.canonicalSlug && (await getGeoreferenceReference(input.canonicalSlug, sourceId))) {
      return {
        kind: "native",
    ...
    const boundedPath = resampleByArc(
      ...
      (a, b) => {
        const east = (((b.longitudeDeg - a.longitudeDeg) * Math.PI) / 180) * EARTH_RADIUS_M * Math.cos((a.latitudeDeg * Math.PI) / 180);
        const north = (((b.latitudeDeg - a.latitudeDeg) * Math.PI) / 180) * EARTH_RADIUS_M;
  ...
  const local = input.packets.map((packet) => ({ x: packet.PositionX, z: packet.PositionZ })).filter((point): point is LocalPoint => finite(point.x) && finite(point.z));
  if (local.length < MIN_SAMPLES || local.length !== input.packets.length) return null;
  ...
  const existing = await getGeoreferenceTransform(input.canonicalSlug, input.gameId, input.trackOrdinal, reference.referenceVersion);
  if (existing) {
    return {
      kind: "derived",
  ...
  const fitSampleCount = Math.min(200, Math.max(MIN_SAMPLES, local.length, referenceEnu.length));
  ...
  if (!hasMinimumSpan(sourceSamples) || !hasMinimumSpan(targetSamples)) return null;
  const fitted = computeAlignment(sourceSamples, targetSamples, {
    allowReverse: false,
    sampleCount: fitSampleCount,
    inputsAreArcSamples: true,
  });
  if (!fitted || fitted.scale < MIN_SCALE || fitted.scale > MAX_SCALE) return null;
...
export async function assignSessionGeoreference(sessionId: number, gameId: GameId): Promise<void> {
  const rows = (await getLapsForSession(sessionId))
    .filter((lap) => lap.isValid && lap.lapTime > 0)
    .sort((first, second) => Number(second.rawFrameCount ?? 0) - Number(first.rawFrameCount ?? 0) || first.lapTime - second.lapTime);
  const adapter = getServerGame(gameId);
  for (const row of rows) {
    const lap = await getLapById(row.id);
    const trackOrdinal = lap?.trackOrdinal;
    if (!lap || lap.gameId !== gameId || trackOrdinal == null || lap.telemetry.length < MIN_SAMPLES) continue;
...
export async function resolveLapGeoreference(input: { canonicalSlug?: string; gameId: GameId; trackOrdinal: number; packets: readonly TelemetryPacket[] }): Promise<LapGeoreference | null> {
  if (input.gameId === "iracing") {
  ...
  const reference = await getLatestGeoreferenceReference(input.canonicalSlug);
  if (!reference || reference.referencePath.length < MIN_SAMPLES) return null;
  const transform = await getGeoreferenceTransform(input.canonicalSlug, input.gameId, input.trackOrdinal, reference.referenceVersion);
