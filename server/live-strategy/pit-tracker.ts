  ...
  async seedFromHistory(trackOrdinal: number, carOrdinal: number, pi: number, gameId: GameId, policy: ServerGameRuntimePolicy["pit"]): Promise<void> {
    ...
    try {
      ...
      const matching = allLaps
        .filter(
          (lap: LapMeta) =>
            ...
            lap.pi === pi &&
            lap.lapTime > 10 &&
            ((seedFuel && isTimedLapEligibilityUsable(lap, "fuel-burn")) || (seedTires && isTimedLapEligibilityUsable(lap, "tire-analysis"))),
        )
      ...
      for (const lapMeta of matching) {
        const fuelEligible = seedFuel && isTimedLapEligibilityUsable(lapMeta, "fuel-burn");
        const tireEligible = seedTires && isTimedLapEligibilityUsable(lapMeta, "tire-analysis");
