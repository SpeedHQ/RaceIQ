...
export async function buildModelEvalDatasetDefinitions(fixture: ParsedModelEvalFixture, repeats = 3): Promise<readonly [ModelEvalDatasetDefinition, ModelEvalDatasetDefinition]> {
  ...
  return [{ id: "raceiq-model-eval-lap-analyst", name: "RaceIQ model eval — lap analyst", targetId: "lap-analyst", scorerIds: ["output-shape", "corner-coverage", "numeric-grounding", "unit-consistency", "code-trajectory-scorer", "check-no-tool-errors"], items: analystItems }, { id: "raceiq-model-eval-compare-engineer", name: "RaceIQ model eval — compare engineer", targetId: "compare-engineer", scorerIds: ["compare-directionality", "unit-consistency", "code-trajectory-scorer", "check-no-tool-errors"], items: compareItems }];
}
...
export async function syncModelEvalDataset(mastra: Mastra, definition: ModelEvalDatasetDefinition): Promise<{ dataset: Dataset; version: number }> {
  ...
  for (const [externalId, payload] of expected) { const old = existing.find((item) => item.externalId === externalId); if (!old) await (dataset as unknown as { addItem: (item: unknown) => Promise<unknown> }).addItem(payload); else { const oldPayload = Object.fromEntries(Object.entries({ externalId: old.externalId, input: old.input, groundTruth: old.groundTruth, expectedTrajectory: old.expectedTrajectory, toolMocks: old.toolMocks, unmockedToolPolicy: old.unmockedToolPolicy, metadata: old.metadata }).filter(([, value]) => value !== undefined)); if (canonical(payload) !== canonical(oldPayload)) await (dataset as unknown as { updateItem: (item: unknown) => Promise<unknown> }).updateItem({ itemId: old.id, ...payload }); } }
