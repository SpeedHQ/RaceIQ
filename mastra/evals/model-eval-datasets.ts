...
import type { Dataset, DatasetItem, DatasetItemPayload } from "@mastra/core/datasets";
...
export async function buildModelEvalDatasetDefinitions(fixture: ParsedModelEvalFixture, repeats = 3): Promise<readonly [ModelEvalDatasetDefinition, ModelEvalDatasetDefinition]> {
  ...
  const lapIds = [analystLap.id, lapA.id, lapB.id];
  ...
  const lapInfo = (lap: CapturedLapWithPackets) => ({ id: lap.id, lapNumber: lap.lapNumber, lapTime: lap.lapTime, isValid: lap.isValid, carOrdinal, trackOrdinal, gameId: config.gameId }); const compareInput = buildInputsComparePrompt(lapInfo(lapA), lapInfo(lapB), comparison, promptSegments);
  ...
  const analystItems = make("lap-analyst", analystInput, analystTruth, [analystLap.id], { expectedTrajectory: { allowRepeatedSteps: false, noRedundantCalls: true, maxSteps: 4 }, blacklistedTools: ["generate_lap_analysis"] });
  ...
  const compareItems = make("compare-engineer", compareInput, compareTruth, [lapA.id, lapB.id], { toolMocks: [mock(lapA.id), mock(lapB.id)], unmockedToolPolicy: "allow", expectedTrajectory: { steps: [{ toolName: "get_lap_analysis", toolArgs: { lapId: lapA.id } }, { toolName: "generate_lap_analysis", toolArgs: { lapId: lapA.id } }, { toolName: "get_lap_analysis", toolArgs: { lapId: lapB.id } }, { toolName: "generate_lap_analysis", toolArgs: { lapId: lapB.id } }], allowRepeatedSteps: false, noRedundantCalls: true, maxSteps: 8 } });
...
export async function syncModelEvalDataset(mastra: Mastra, definition: ModelEvalDatasetDefinition): Promise<{ dataset: Dataset; version: number }> {
  ...
  const existing: DatasetItem[] = []; page = 0; let hasMore = true; while (hasMore) { const listed = await dataset.listItems({ page, perPage: 100 }); const items: DatasetItem[] = Array.isArray(listed) ? listed : listed.items; existing.push(...items); hasMore = !Array.isArray(listed) && listed.pagination.hasMore; page++; }
  ...
  for (const [externalId, payload] of expected) { const old = existing.find((item) => item.externalId === externalId); if (!old) await dataset.addItem(payload); else if (canonical(payload) !== canonical({ externalId: old.externalId, input: old.input, groundTruth: old.groundTruth, expectedTrajectory: old.expectedTrajectory, toolMocks: old.toolMocks, unmockedToolPolicy: old.unmockedToolPolicy, metadata: old.metadata })) await dataset.updateItem({ itemId: old.id, ...payload }); }
