...
**Deployment:** The Docker containerisation plan (see `2026-03-20-docker-containerisation-design.md`) must be updated to include Python 3 + `edge-tts` in the server image. A multi-stage build can keep the image lean. If Python is not available at runtime, the endpoint returns 503 with `{ error: "TTS not available — edge-tts not installed" }`.
