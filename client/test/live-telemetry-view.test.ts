...
const positionSchema = { ...schema, definitions:["motion.position-x","motion.position-z"].map(semanticId=>({semanticId,unit:"m",mappingStatus:"direct",schemaVersion:"1",limitations:[]})) } as LiveTelemetrySchemaMessageV1;
...
describe("live telemetry view",()=>{
 ...
 it("requires both coordinates for a live position",()=>{const complete=buildLiveTelemetryView(positionSchema,frame([12,34]))!; expect(complete.motion.position).toEqual({x:12,z:34}); const partial=buildLiveTelemetryView(positionSchema,frame([12,null],"s",{1:"missing"}))!; expect(partial.motion.position).toBeUndefined(); expect(partial.stateBySemanticId["motion.position-z"]).toBe("missing");});
});
