...
test("renders limitations and canonical channels", () => {
  ...
  const markup = renderToStaticMarkup(
    createElement(MotecImportNote, {
      result: {
        ...
        limitations: [
          "Racing line is estimated from speed and lateral G force because MoTeC does not record track position. Yaw rate controls car direction and is only used for the line if lateral G is unavailable. The estimate may drift.",
        ],
      },
      onClose: () => {},
    }),
  ...
  ).toBeLessThan(markup.indexOf("Canonical channels"));
