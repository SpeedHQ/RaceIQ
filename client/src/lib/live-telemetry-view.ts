...
export interface LiveCompetitorView { position?: number; name?: string; gapToAheadS?: number; gapToLeaderS?: number; tireCompound?: string | number; tireAge?: number; pitStatus?: "none" | "pitting" | "in-pit-area"; onPitRoad?: boolean; pitStops?: number; lastS1S?: number; lastS2S?: number; lastS3S?: number; }
...
export interface LiveTelemetryView {
 ...
 inputs: { throttle?:number; brake?:number; clutch?:number; handbrake?:number; steering?:number; gear?:number };
 engine: { rpm?:number; idleRpm?:number; maxRpm?:number; powerW?:number; torqueNm?:number; boost?:number };
 fuel: { remainingVolumeL?: number; remainingFraction?: number; remainingPercent?: number; capacityL?: number };
 ...
 tires: { temperatureC?:WheelValues<number>; wear?:WheelValues<number>; blistering?:WheelValues<number>; pressurePsi?:WheelValues<number>; slipAngleRad?:WheelValues<number>; slipRatio?:WheelValues<number>; combinedSlip?:WheelValues<number>; rotationRadS?:WheelValues<number>; suspensionNormalized?:WheelValues<number>; brakeTemperatureC?:WheelValues<number>; brakePadRemainingMm?:WheelValues<number>; radiusM?:WheelValues<number>; surfaceRumble?:WheelValues<number>; puddleDepth?:WheelValues<number>; onRumbleStrip?:WheelValues<number>; compound?:string|number };
...
export function buildLiveTelemetryView(schema: LiveTelemetrySchemaMessageV1, frame: LiveTelemetryFrameMessageV1): LiveTelemetryView | undefined {
 ...
 [[grouped.identity,"carOrdinal","identity.car-ordinal"],[grouped.identity,"trackOrdinal","identity.track-ordinal"],[grouped.identity,"carClass","identity.car-class"],[grouped.identity,"performanceIndex","identity.car-performance-index"],[grouped.identity,"drivetrainType","identity.drivetrain-type"],[grouped.motion,"speedMps","motion.speed"],[grouped.motion,"distanceM","timing.distance-traveled"],[grouped.motion,"positionX","motion.position-x"],[grouped.motion,"positionZ","motion.position-z"],[grouped.motion,"yaw","motion.yaw"],[grouped.motion,"roll","motion.roll"],[grouped.motion,"pitch","motion.pitch"],[grouped.motion,"accelerationX","motion.acceleration-x"],[grouped.motion,"accelerationZ","motion.acceleration-z"],[grouped.inputs,"throttle","inputs.throttle"],[grouped.inputs,"brake","inputs.brake"],[grouped.inputs,"clutch","inputs.clutch"],[grouped.inputs,"handbrake","inputs.handbrake"],[grouped.inputs,"steering","inputs.steering"],[grouped.inputs,"gear","inputs.gear"],[grouped.engine,"rpm","engine.current-engine-rpm"],[grouped.engine,"idleRpm","engine.engine-idle-rpm"],[grouped.engine,"maxRpm","engine.engine-max-rpm"],[grouped.engine,"powerW","engine.power"],[grouped.engine,"torqueNm","engine.torque"],[grouped.engine,"boost","engine.boost"],[grouped.fuel,"remainingVolumeL","fuel.remaining-volume"],[grouped.fuel,"remainingFraction","fuel.remaining-fraction"],[grouped.fuel,"remainingPercent","fuel.remaining-percent"],[grouped.fuel,"capacityL","fuel.capacity"],[grouped.timing,"lapNumber","timing.lap-number"],[grouped.timing,"currentLapS","timing.current-lap"],[grouped.timing,"lastLapS","timing.last-lap"],[grouped.timing,"bestLapS","timing.best-lap"],[grouped.timing,"lapFraction","timing.lap-fraction"],[grouped.timing,"racePosition","race.race-position"],[grouped.weather,"kind","weather.weather-type"],[grouped.weather,"airTemperatureC","weather.air-temp"],[grouped.weather,"trackTemperatureC","weather.track-temp"]].forEach(([o,k,id])=>void put(o,k,id));
 ...
 if (grouped.motion.positionX !== undefined || grouped.motion.positionZ !== undefined) grouped.motion.position={x:grouped.motion.positionX ?? 0,z:grouped.motion.positionZ ?? 0};
 if (grouped.motion.yaw !== undefined || grouped.motion.roll !== undefined || grouped.motion.pitch !== undefined) grouped.motion.attitude={yaw:grouped.motion.yaw ?? 0,roll:grouped.motion.roll ?? 0,pitch:grouped.motion.pitch ?? 0};
 ([ ["temperatureC","tire.temperature.average"],["wear","tires.tire-wear"],["blistering","tires.blistering"],["pressurePsi","tires.tire-pressure"],["slipAngleRad","tires.tire-slip-angle"],["slipRatio","tires.tire-slip-ratio"],["combinedSlip","tires.tire-combined-slip"],["rotationRadS","tires.wheel-rotation-speed"],["suspensionNormalized","suspension.norm-suspension-travel"],["brakeTemperatureC","brakes.brake-temp"],["brakePadRemainingMm","damage.brake-pad-wear"],["radiusM","tires.tire-radius"],["surfaceRumble","tires.surface-rumble"],["puddleDepth","tires.wheel-in-puddle-depth"],["onRumbleStrip","tires.wheel-on-rumble-strip"]] as const).forEach(([k,id])=>{const v=w(id);if(v)grouped.tires[k]=v;});
 const competitorFields: [string,string][] = [["position","race.competitor.position"],["name","race.competitor.driver-name"],["gapToAheadS","timing.competitor.gap-to-ahead"],["gapToLeaderS","timing.competitor.gap-to-leader"],["tireCompound","tires.competitor.compound"],["tireAge","tires.competitor.age"],["pitStatus","race.competitor.pit-status"],["onPitRoad","race.competitor.on-pit-road"],["pitStops","race.competitor.pit-stops"],["lastS1S","timing.sector.competitor-last.s1"],["lastS2S","timing.sector.competitor-last.s2"],["lastS3S","timing.sector.competitor-last.s3"]];
 ...
 ] as const).forEach(([key,id])=>void f1Put(key,id));
