...
export function drawInputsHUD(ctx: CanvasRenderingContext2D, w: number, h: number, pA: SemanticTelemetrySample | null, pB: SemanticTelemetrySample | null) {
  ...
  const normalizedPedal = (value: number | undefined) => {
    const input = value ?? 0;
    return input > 1 ? input / 255 : input;
  };
  const normalizedSteering = (value: number | undefined) => {
    const input = value ?? 0;
    return Math.abs(input) > 1 ? input / 127 : input;
  };


  const brakeA = normalizedPedal(num(pA!, "inputs.brake"));
  const brakeB = normalizedPedal(num(pB!, "inputs.brake"));
  ...
  const steerA = normalizedSteering(num(pA!, "inputs.steer"));
  ...
  // --- Steering wheel B ---
  const steerB = normalizedSteering(num(pB!, "inputs.steer"));
  ...
  const throttleA = normalizedPedal(num(pA!, "inputs.accel"));
  const throttleB = normalizedPedal(num(pB!, "inputs.accel"));
