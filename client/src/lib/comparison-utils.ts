...
export function drawTrackCanvas(
  ...
  if (followCar && zoom && hoveredDistance != null) {
    ...
    const x = traces.positionXA[index],
      z = traces.positionZA[index],
      yaw = traces.yawA[index];
  ...
  const drawPath = (points: Point[], color: string, width: number, close = false) => {
    ...
    ctx.beginPath();
    const [x, y] = toCanvas(points[0].x, points[0].z);
    ctx.moveTo(x, y);
    for (let i = 1; i < points.length; i++) {
      const [px, py] = toCanvas(points[i].x, points[i].z);
      ctx.lineTo(px, py);
    }
  ...
  if (boundaries && boundaries.leftEdge.length > 1 && boundaries.rightEdge.length > 1) {
    ...
    ctx.beginPath();
    const [sx, sy] = toCanvas(surface[0].x, surface[0].z);
    ctx.moveTo(sx, sy);
    for (const p of surface.slice(1)) {
  ...
  if (!hideOutline) {
    ...
    if (outline[0]) {
      const [x, y] = toCanvas(outline[0].x, outline[0].z);
      ctx.beginPath();
      ctx.arc(x, y, zoom ? 5 : 4, 0, Math.PI * 2);
  ...
  const drawLine = (xs: number[], zs: number[], color: string) => {
    ...
    for (let i = 0; i < xs.length; i++) {
      ...
      const [x, y] = toCanvas(telX(xs[i]), zs[i]);
      if (!moved) {
        ctx.moveTo(x, y);
        moved = true;
      } else ctx.lineTo(x, y);
  ...
  if (hoveredDistance != null) {
    ...
    const dot = (p: Point | null, color: string, yaw?: number) => {
      ...
      const [x, y] = toCanvas(p.x, p.z);
      ctx.beginPath();
      ctx.arc(x, y, zoom ? 7 : 5, 0, Math.PI * 2);
      ctx.fillStyle = color;
      ctx.fill();
      if (zoom && yaw != null) {
        ctx.beginPath();
        ctx.moveTo(x, y);
        ctx.lineTo(x - Math.sin(yaw) * 22, y + Math.cos(yaw) * 22);
  ...
  if (showSegmentMarkers && segmentPoints && !zoom)
    for (const point of segmentPoints) {
      const [x, y] = toCanvas(point.x, point.z);
      ctx.beginPath();
      ctx.arc(x, y, 3.5, 0, Math.PI * 2);
      ctx.fillStyle = point.type === "corner" ? "var(--track-corner-marker)" : "var(--track-straight-marker)";
      ctx.fill();
    }
...
export function drawInputsHUD(ctx: CanvasRenderingContext2D, w: number, h: number, traces: AlignedTrace, index: number) {
  ...
  const barW = 14,
    ...
    gap = 16,
    y = h - barH - 30,
    total = 2 * (barW * 2 + 4) + wheelR * 4 + gap * 4;
  let x = (w - total) / 2;
  const bar = (value: number, color: string, border: string) => {
    ctx.fillStyle = "var(--app-surface-alt)";
    ctx.fillRect(x, y, barW, barH);
    ctx.fillStyle = color;
    ctx.fillRect(x, y + barH * (1 - value), barW, barH * value);
    ctx.strokeStyle = border;
    ctx.strokeRect(x, y, barW, barH);
    x += barW + 4;
  };
  bar(traces.brakeA[index] ?? 0, "var(--ch-brake)", COLOR_A);
  bar(traces.brakeB[index] ?? 0, "var(--ch-brake)", COLOR_B);
  x += gap;
  const wheel = (steer: number, gear: number, color: string) => {
    const cx = x + wheelR,
      cy = y + barH / 2;
    ctx.beginPath();
    ...
    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ctx.lineTo(cx + Math.sin(steer / 127) * wheelR, cy - Math.cos(steer / 127) * wheelR);
    ctx.strokeStyle = "var(--app-text)";
    ctx.lineWidth = 2;
    ctx.stroke();
    ctx.fillStyle = "var(--app-text)";
    ctx.font = "var(--font-weight-bold) var(--text-app-heading) var(--font-mono)";
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText(gear > 0 ? String(gear) : gear === 0 ? "N" : "R", cx, cy);
    ctx.textBaseline = "alphabetic";
    x += wheelR * 2 + gap;
  };
  wheel(traces.steerA[index] ?? 0, traces.gearA[index] ?? 0, COLOR_A);
  wheel(traces.steerB[index] ?? 0, traces.gearB[index] ?? 0, COLOR_B);
  x += gap;
  bar(traces.throttleA[index] ?? 0, "var(--ch-throttle)", COLOR_A);
  bar(traces.throttleB[index] ?? 0, "var(--ch-throttle)", COLOR_B);
}
