...
/** Convert RaceIQ's canonical 0–1 control-input ratio to a clamped display percentage. */
export function controlInputPercent(ratio: number | undefined): number {
  return Math.min(1, Math.max(0, ratio ?? 0)) * 100;
}
