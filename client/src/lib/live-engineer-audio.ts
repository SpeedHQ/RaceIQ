...
export const AUDIO_START_LEAD_MS = 250;
export const CHAINED_LAP_PAUSE_MS = 300;
export function getSegmentPauseMs(segmentId: string): number { return segmentId === "lap.tail.flat" || segmentId.startsWith("lap.tenth.") ? CHAINED_LAP_PAUSE_MS : 0; }
...
interface QwenCatalog { catalogVersion: string; sampleRate: number; channels: number; assembly?: { mode: string; gapMs: number }; listeningStatus?: string; approvals?: { fixed?: Record<string, string>; recipes?: Array<{ status: string }> }; clips: CatalogSegment[]; fullLines: CatalogFullLine[]; oracles?: CatalogOracle[]; }
...
export interface LiveEngineerAudioOptions { fetchImpl?: (...args: Parameters<typeof fetch>) => Promise<Response>; audioContext?: AudioContext; radioEffect?: boolean; catalog?: { url: string; version: string }; }
...
export class LiveEngineerAudioPlayer {
  ...
  private async playEntries(segments: readonly CatalogSegment[], volume: number, joinGapMs: number, generation: number): Promise<void> { this.assertCurrent(generation); this.setVolume(volume); if (this.context.state === "suspended") { try { await this.context.resume(); } catch { throw new LiveEngineerAudioError("audio-blocked", "Audio context blocked"); } } this.assertCurrent(generation); const buffers = await Promise.all(segments.map((segment) => this.load(segment, generation))); this.assertCurrent(generation); if (this.context.state === "suspended") { try { await this.context.resume(); } catch { throw new LiveEngineerAudioError("audio-blocked", "Audio context blocked"); } } if (this.context.state !== "running") throw new LiveEngineerAudioError("audio-blocked", "Audio context is not running"); this.stop(); const scheduledGeneration = this.generation; this.setVolume(volume); const contextLeadMs = Math.max(AUDIO_START_LEAD_MS, ((this.context.baseLatency ?? 0) + (this.context.outputLatency ?? 0)) * 1000 * 2); const startTime = this.context.currentTime + contextLeadMs / 1000; const gap = joinGapMs / 1000; let previousStart = startTime; let previousDuration = 0; return new Promise<void>((resolve) => { this.completion = resolve; buffers.forEach((buffer, index) => { const sourceStart = index === 0 ? startTime : previousStart + previousDuration + gap + getSegmentPauseMs(segments[index - 1]!.segmentId) / 1000; const source = this.context.createBufferSource(); source.buffer = buffer; source.connect(this.gainNode); source.start(sourceStart); previousStart = sourceStart; previousDuration = buffer.duration; this.active.add(source); source.onended = () => { this.active.delete(source); if (scheduledGeneration === this.generation && !this.active.size) { this.completion?.(); this.completion = null; } }; }); }); }
