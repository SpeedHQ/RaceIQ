    ...
    <fieldset className="space-y-2" disabled={disabled}>
      ...
      <div className="grid grid-cols-2 gap-2">
        {(["mine", "others"] as const).map((option) => (
          <label
            key={option}
            className={`flex cursor-pointer items-center gap-2 rounded border px-3 py-2 text-sm transition-colors ${value === option ? "border-app-accent bg-app-accent/10 text-app-text" : "border-app-border text-app-text-muted hover:border-app-accent/60"}`}
          >
