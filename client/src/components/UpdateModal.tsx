    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-app-bg/60 p-4" onClick={isUpdating ? undefined : onClose}>
      <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-lg border border-app-border bg-app-bg shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-app-border">
          ...
          {!isUpdating && (
            <Button type="button" variant="app-ghost" size="icon-sm" onClick={onClose} className="!h-auto !w-auto !p-1.5 text-app-text-muted hover:text-app-text" aria-label={m.common_close()}>
              <X className="size-4" />
            </Button>
          )}
        </div>
          ...
          {!stage && !error && (
            <>
                  ...
                  return (
                    <div className="max-h-52 overflow-y-auto space-y-3">
                      ...
                      {older.length > 0 && !showAllReleases && (
                        <Button type="button" variant="app-ghost" size="app-sm" onClick={() => setShowAllReleases(true)} className="!h-auto !p-0 text-xs text-app-accent hover:underline">
              ...
              <div className="flex justify-end gap-3">
                <Button type="button" variant="app-primary" size="app-md" onClick={handleInstall}>
