...
export function GearRatioChart({ ratios, finalDrive, topSpeedKph, topSpeedMph, maxRpm = 8000 }: { ratios: number[]; finalDrive: number; topSpeedKph?: number; topSpeedMph?: number; maxRpm?: number }) {
  ...
  const referenceTopSpeedKph = topSpeedKph ?? (topSpeedMph ? topSpeedMph * 1.60934 : undefined);
  ...
  const sx = (value: number) => Math.min((value / maxSpeed) * chartWidth, chartWidth);
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${width} ${height}`} className="block w-full text-app-text-muted" aria-label="Gear ratio speed chart">
      ...
      <rect x={pad.left} y={pad.top} width={chartWidth} height={chartHeight} fill="currentColor" fillOpacity="0.03" />

      {rpmGrids.map((rpm) => (
        <g key={rpm}>
          <line x1={pad.left} y1={pad.top + sy(rpm)} x2={pad.left + chartWidth} y2={pad.top + sy(rpm)} stroke="currentColor" strokeOpacity="0.1" />
          <text x={pad.left - 4} y={pad.top + sy(rpm) + 3} textAnchor="end" fontSize="7" fill="currentColor" fillOpacity="0.45">
            {rpm / 1000}
          </text>
        </g>
      ))}
