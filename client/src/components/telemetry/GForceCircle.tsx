 ...
 * Live view remains supported for native telemetry; Analyse passes a semantic frame.
 */
export function GForceCircle({ frame, view }: { frame?: SemanticAnalysisFrame; view?: LiveTelemetryView }) {
  const accelerationX = view?.motion.acceleration?.x ?? number(frame, "motion.acceleration-x");
  const accelerationZ = view?.motion.acceleration?.z ?? number(frame, "motion.acceleration-z");
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const size = 110;
  const maxG = 2.5;

  function number(current: SemanticAnalysisFrame | undefined, id: keyof SemanticAnalysisFrame["values"]): number | null {
  ...
  useEffect(() => {
    ...
    if (accelerationX == null || accelerationZ == null) return;
    // Acceleration values are inverted relative to felt G-force.
  ...
  const latG = accelerationX == null ? null : -accelerationX / 9.81;
  const lonG = accelerationZ == null ? null : -accelerationZ / 9.81;
  ...
  return (
    <div className="flex flex-col items-center gap-0.5 shrink-0" style={{ width: size }}>
      ...
      <div className="flex gap-2 text-app-nano font-mono text-app-text-secondary tabular-nums">
        <span className="w-6 text-right">{latG == null ? "—" : `${latG >= 0 ? " " : ""}${latG.toFixed(1)}`}</span>
        <span className="w-6 text-right">{lonG == null ? "—" : `${lonG >= 0 ? " " : ""}${lonG.toFixed(1)}`}</span>
      </div>
    </div>
  );
}
