 ...
 */
export function PitEstimate({ view, pit }: PitEstimateProps) {
  ...
  const wears = view.tires.wear ? [view.tires.wear.fl, view.tires.wear.fr, view.tires.wear.rl, view.tires.wear.rr] : undefined;
  const healthAvailable = hasTireHealthData(wears, analysis.tireHealth);
  const fuelFillRatio = fuel?.fillRatio;
  const fuelPct = fuelFillRatio === undefined ? undefined : fuelFillRatio * 100;
  const isFuelCritical = fuel === undefined ? false : fuel.fillRatio === undefined ? fuel.amount < 5 : fuel.fillRatio < 0.2;
  const isFuelWarning = fuel !== undefined && !isFuelCritical && (fuel.fillRatio === undefined ? fuel.amount < 15 : fuel.fillRatio < 0.4);
  const fuelColor = fuel === undefined ? "var(--status-unavailable)" : severityColor(isFuelCritical ? 3 : isFuelWarning ? 1 : 0);
  ...
  const tireData = tireLabels.map((label, i) => {
    const wear = wears?.[i];
    const health = healthAvailable && wear !== undefined ? (1 - wear) * 100 : null;
  ...
  const pitStatus = telemetryModel.pitStatus ? String(view.competitors[0]?.pitStatus ?? "") : "";
        ...
        <div className="py-1">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className={`text-2xl font-mono font-black tabular-nums leading-none ${fuel?.unit === "L" ? "w-20" : "w-14"} text-right`} style={{ color: fuelColor }}>
              {fuel === undefined ? "—" : fuel.amount.toFixed(fuel.unit === "L" ? 1 : 0)}
              {fuel?.unit ?? ""}
            </div>
          </div>
        </div>
