...
function TrackDetail({ track, onBack, initialTab, navigate }: { track: TrackInfo; onBack: () => void; initialTab?: string; navigate: ReturnType<typeof useNavigate> }) {
  ...
  const validTabs = ["laps", "tunes", "debug"] as const;
  type Tab = typeof validTabs[number];
  const [activeTab, setActiveTabState] = useState<Tab>(
    validTabs.includes(initialTab as Tab) ? (initialTab as Tab) : "laps"
  );
    ...
    <div className="p-4 overflow-auto h-full">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="flex items-center gap-1 border-b border-app-border">
          {(["laps", "tunes", "debug"] as const).map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`text-app-label uppercase tracking-wider px-3 py-1.5 -mb-px border-b-2 transition-colors ${
                activeTab === tab
                  ? tab === "debug" ? "border-amber-500 text-amber-500" : "border-app-accent text-app-accent"
                              ...
                              <tr key={lap.lapId} className={`border-b border-app-border/50 hover:bg-app-surface-alt/30 ${selectedLaps.has(lap.lapId) ? "bg-cyan-500/5" : ""}`}>
                                ...
                                <td className="px-2 py-1.5 text-right">
                                  <div className="flex items-center justify-end gap-1">
                                    <button
                                      onClick={() => navTo({ to: "/analyse", search: { track: track.ordinal, car: lap.carOrdinal, lap: lap.lapId } })}
                      ...
                      {selectedLaps.size > 0 && (
                        <div className="flex items-center gap-2 p-2 bg-app-surface-alt/50 rounded-lg border border-app-border">
                          ...
                          {selectedLaps.size === 2 && (() => {
                            ...
                            return (
                              <button
                                onClick={() => navTo({ to: "/compare", search: { track: track.ordinal, lapA, lapB, carA: trackLaps.find((l) => l.lapId === lapA)?.carOrdinal, carB: trackLaps.find((l) => l.lapId === lapB)?.carOrdinal } })}
