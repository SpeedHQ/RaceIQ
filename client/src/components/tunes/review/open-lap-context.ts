    ...
    lines.push(
      `Tyres (end of lap): ${cornerKeys
        .map((corner) => {
          ...
          return `${corner} temp ${snapshot.tempC.toFixed(0)}°C, wear ${snapshot.wear.toFixed(0)}%, pressure ${snapshot.pressure.toFixed(1)}psi, brake ${snapshot.brakeTemp.toFixed(0)}°C`;
        })
        .join("; ")}`,
    );
