...
export function TrackCarAnalyseReviewPage({ gameId, trackOrdinal, carOrdinal, sessionId }: { gameId: GameId; trackOrdinal?: number; carOrdinal?: number; sessionId?: number }) {
  ...
  return <div className="flex flex-col gap-3 p-3"><div className="px-1 text-sm text-app-text-muted">{resolvedTrackName} · {resolvedCarName} · {sessionId != null ? "Selected session" : `${groupSessions.length} sessions`} · {sessionId != null ? selectedSession?.lapCount ?? evaluationLaps.length : groupSessions.reduce((total, session) => total + (session.lapCount ?? 0), 0)} laps</div><div className="rounded-lg border border-app-border"><SessionReviewDashboard gameId={gameId} stayOnSessionReview autoSelectLap={false} laps={evaluationLaps} trackName={resolvedTrackName} onBack={backToPicker} lineSpread={sessionLineSpread ?? null} /></div></div>;
}
