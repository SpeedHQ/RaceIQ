...
import { bandColor, buildSectorRanges, CORNERS, CornerBars, sectorIndexForTelemetryIndex, type CornerKey, METRICS, metricDisplayValue, type MetricKey, tuneMetricValue, tuneMetricsFor } from "@/components/tunes/SectorRangeBreakdown";
      ...
      return CORNERS.map((corner, index) => {
        ...
        return {
          ...
          value:
            value === undefined
              ? "—"
              : `${metric.quantity === "temperature" ? units.temp(value).toFixed(1) : value.toFixed(metric.key === "wear" ? 0 : 1)} ${metric.quantity === "temperature" ? units.tempLabel : metric.unit}`,
          ...
          <div className="border-b border-app-border">
              ...
              <div className="min-w-0 divide-y divide-app-border">
                {Array.from({ length: sectorCount }, (_, i) => `S${i + 1}`).map((label, i) => (
                  <div key={label} className="grid grid-cols-[4.5rem_minmax(0,1fr)] items-center gap-3 p-3">
                    ...
                    {ranges ? (
                      <CornerBars
                        ranges={ranges.sectors[i]}
                        domain={ranges.domain}
                        metric={metric}
                        cursor={hoverPos?.sector === i ? cursor : undefined}
            ...
            {ranges && (
              <div className="px-4 py-1.5 text-app-compact text-app-text-dim border-t border-app-border">
                {metric.label}: bars span min→max, tick = average · shared scale {Math.round(metricDisplayValue(metric, ranges.domain[0], units.temperatureUnit))}–
                {Math.round(metricDisplayValue(metric, ranges.domain[1], units.temperatureUnit))} {metric.quantity === "temperature" ? units.tempLabel : metric.unit}
              </div>
            )}
          </div>
        )}
