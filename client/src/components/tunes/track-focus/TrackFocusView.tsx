...
import { trackFractionRange } from "../../../lib/aligned-telemetry-fidelity";
...
export function TrackFocusView({
  ...
  const visibleLaneRange = useMemo(() => {
    if (!zoom.visibleRange || !zoom.data) return null;
    return trackFractionRange(zoom.data);
  }, [zoom.data, zoom.visibleRange]);
...
export function TrackFocusViewInner({
  ...
  const [tooltipMode, setTooltipMode] = useState<TooltipDisplayMode>("per-lap");
    ...
    <div className="flex min-h-full flex-col gap-4 px-4 pb-4" onPointerMove={(event) => {
      ...
      <div className="grid min-w-0 grid-cols-1 gap-4 @5xl/workspace:grid-cols-[460px_minmax(0,1fr)]">
        ...
        <div className="flex min-w-0 flex-col">
          <div className="flex flex-none flex-wrap items-center gap-1 bg-app-bg pt-4 @5xl/workspace:sticky @5xl/workspace:top-[2.8125rem] @5xl/workspace:z-20">
            {TABS.map((t) => (
              <Button
                key={t}
                variant="app-ghost"
          ...
          <div className="min-w-0" onMouseLeave={() => setZoomActive(false)}>
            <div className="bg-app-bg/95 @5xl/workspace:sticky @5xl/workspace:top-[5.1875rem] @5xl/workspace:z-10">
              <TrackZoomHint />
              <TurnMarkers corners={effectiveCorners.corners} cornerFracs={effectiveCorners.fracs} cursorFrac={cursorFrac} nominalSpanMeters={nominalSpanMeters} visibleRange={visibleLaneRange} />
...
function TurnMarkers({
  ...
  const markers = useMemo(() => {
    ...
    const span = visibleRange ? Math.max(1e-9, visibleRange.end - start) : 1;
    const labels = cornerFracs
      .map((frac, index) => ({ frac: (frac - start) / span, label: corners[index]?.label ?? `T${index + 1}`, sourceFrac: frac }))
      .filter(({ sourceFrac }) => sourceFrac >= start && sourceFrac <= start + span);
    ...
    return groups.map(({ start: groupStart, end }) => ({
      frac: (labels[groupStart].frac + labels[end].frac) / 2,
      label: groupStart === end ? labels[groupStart].label : `${labels[groupStart].label}–${labels[end].label.replace(/^T/, "")}`,
    }));
  }, [corners, cornerFracs, visibleRange, width]);

  const cursorPosition = cursorFrac == null ? null : visibleRange ? (cursorFrac - visibleRange.start) / Math.max(1e-9, visibleRange.end - visibleRange.start) : cursorFrac;
  ...
  return (
    <div ref={ref} className="px-1" aria-label="Track turns">
      ...
      <div className="relative h-5 border-t border-app-border/50" aria-label="Track position">
        {cursorPosition != null && cursorFrac != null && (
          <span
            className="absolute top-0 text-app-caption text-app-accent font-mono tabular-nums whitespace-nowrap"
            style={{
              left: `clamp(44px, ${Math.max(0, Math.min(1, cursorPosition)) * 100}%, calc(100% - 44px))`,
