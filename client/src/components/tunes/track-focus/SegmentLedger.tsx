...
interface SegmentLedgerProps {
  ...
  /** Track-fraction scope from active telemetry zoom. */
 ...
 */
export function SegmentLedger({ traces, primaryLapId, cornerFracs, corners, cursorFrac, onCursorFrac, onHoverPoints, onHoverRange, visibleRange }: SegmentLedgerProps) {
  ...
  const scopedRows = useMemo(() => {
    ...
    return rows.filter((row) => row.corner.distanceEnd >= visibleRange.start && row.corner.distanceStart <= visibleRange.end);
  }, [rows, visibleRange]);
