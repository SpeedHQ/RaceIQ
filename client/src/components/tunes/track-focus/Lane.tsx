...
function fitDomain(domain: [number, number], series: LaneSeries[], visibleRange: { start: number; end: number } | null | undefined): [number, number] {
  const start = visibleRange?.start ?? 0;
  const end = visibleRange?.end ?? 1;
  let min = Infinity;
  let max = -Infinity;
  for (const item of series) {
    ...
    for (let i = 0; i < values.length; i++) {
      ...
      if (x[i] >= start && x[i] <= end && Number.isFinite(value)) {
  ...
  const pad = observedRange > 0 ? observedRange * 0.08 : Math.max(Math.abs(min) * 0.05, (domain[1] - domain[0]) * 0.02, 0.001);
  let fitted: [number, number] = [min - pad, max + pad];
  if (domain[0] < 0 && domain[1] > 0 && min <= 0 && max >= 0) {
    fitted = [Math.min(fitted[0], 0), Math.max(fitted[1], 0)];
  }
  return fitted;
}
...
export function Lane({
  ...
  const fittedDomain = useMemo(() => fitDomain(domain, series, visibleRange), [domain, series, visibleRange?.end, visibleRange?.start]);
  ...
  }, [domain, fittedDomain, height, onRangeSelect, onZoomOut, series.length, styleKey, visibleRange?.end, visibleRange?.start]);
  ...
  return (
    <div className="relative isolate overflow-hidden" onPointerEnter={() => onHoverChange?.(true)} onPointerLeave={handlePointerLeave}>
