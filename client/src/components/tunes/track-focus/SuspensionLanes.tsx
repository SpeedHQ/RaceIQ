      ...
      Object.fromEntries(
        CORNERS.map((corner) => [
          ...
          [
            ...
            ...lapsWithTrace.filter((trace) => trace.lapId === primaryLapId).map((trace): LaneSeries => ({ x: trace.frac, values: trace.suspTravel![corner.key], color: "var(--app-accent)", width: 1.8 })),
          ],
        ]),
      ...
      {CORNERS.map((c) => (
        <div key={c.key} className="space-y-1">
          ...
          <Lane
            ...
            tooltip={(f) => {
              const samples = lapsWithTrace.map((lap) => ({ lap, value: lap.suspTravel![c.key][indexAtFrac(lap, f)] }));
              if (tooltipMode === "per-lap") {
                return (
                  <div className="space-y-0.5 font-mono tabular-nums">
                    <ChartSpread values={samples.map((sample) => sample.value)} />
                    {[...samples].sort((a, b) => b.value - a.value).map(({ lap, value }) => (
                      <div key={lap.lapId}><span className={lap.lapId === primaryLapId ? "text-app-accent" : "text-app-text-muted"}>L{lap.lapNumber}</span>: {value.toFixed(2)}</div>
                    ))}
                  </div>
                );
              }
              ...
              return <div className="space-y-0.5 font-mono tabular-nums"><ChartSpread values={samples.map((sample) => sample.value)} />{stats.map(([label, value]) => <div key={label} className={label === "primary" ? "text-app-accent" : "text-app-text-muted"}>{label}: <span className="text-app-text">{value == null ? "—" : value.toFixed(2)}</span></div>)}</div>;
            }}
