...
export function GgScatter({ traces, primaryLapId, cursorFrac }: GgScatterProps) {
  ...
  // Friction-circle plot must stay square; keeping a wide canvas lets CSS
  // stretch its backing surface on analyse panes.
  const plotWidth = Math.min(bw, H);
  const cx = plotWidth / 2;
  const cy = H / 2;
  const r = (plotWidth / 2 - 16) / G_RANGE;
  const px = (latG: number) => cx + latG * r;
  const py = (longG: number) => cy - longG * r;

  useEffect(() => {
    const canvas = staticCanvasRef.current;
    if (!canvas || plotWidth <= 0 || withG.length === 0) return;
    const ctx = getSemanticCanvasContext(canvas);
    if (!ctx) return;
    syncCanvasSize(canvas, plotWidth, H, window.devicePixelRatio || 1, false);
    ctx.setTransform(canvas.width / plotWidth, 0, 0, canvas.height / H, 0, 0);
    ctx.clearRect(0, 0, plotWidth, H);
    ...
    ctx.fillText("right", plotWidth - 30, cy - 4);
  ...
  }, [primaryLapId, plotWidth, cursorActive, cx, cy, r, withG]);
  ...
  }, [primaryLapId, plotWidth, cursorFrac, cx, cy, r, withG]);
  ...
  return (
    <div ref={wrapRef} className="space-y-1">
      ...
      <div className="relative" style={{ width: plotWidth, height: H }}>
