              ...
              <td className="px-3 py-1 text-left">
                <div className="flex items-center gap-1">
                  <span className="w-[130px] shrink-0 flex items-center gap-2">
                    {status && (
                      <span
                        className={`text-[10px] uppercase tracking-wider truncate ${excluded ? "text-app-text-dim" : isPitStatus ? "text-status-warning" : "text-status-danger"}`}
                    ...
                    {reason === "chosen" && (
                      <span className="text-[10px] uppercase tracking-wider text-status-success" title={`Used for evaluation — one of the fastest ${REVIEW_LAP_CAP} clean laps this analysis reads`}>
                        Eval
                      </span>
                    )}
