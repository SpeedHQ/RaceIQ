              ...
              return (
                <div key={l.id} className="grid gap-x-2 px-3 py-1.5 items-center" style={{ gridTemplateColumns }}>
                  ...
                  {sectorLabels.map((label, index) => {
                    ...
                    return (
                      <span key={label} className={`text-sm font-mono tabular-nums text-right ${paceEligible ? sectorColor(time, bestSectors[index]) : "text-app-text-muted"}`}>
                  ...
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <Button variant="app-danger" size="app-sm" onClick={() => deleteLap.mutate(l.id)} className="!px-1 !py-0.5" aria-label={m.common_delete()} title={m.common_delete()}>
                      ×
                    </Button>
                  </div>
                </div>
