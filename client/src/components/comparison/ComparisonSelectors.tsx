...
export function buildComparisonLapOption(lap: LapMeta, locale?: "en" | "de") {
  return {
    value: String(lap.id),
    label: [`${m.compare_lap_label({}, { locale })} ${lap.lapNumber} — ${formatLapTime(lap.lapTime)} — ${lap.ownership === "others" ? m.import_ownership_others({}, { locale }) : m.import_ownership_mine({}, { locale })}${!lap.isValid ? " (inv)" : ""}`, lapStatusLabel(lap, "issues")].filter(Boolean).join(" · "),
  };
}
    ...
    <div className="flex shrink-0 flex-wrap items-end gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {lapAId != null && (
            <span className="shrink-0 rounded border border-app-border px-1.5 py-0.5 text-xs font-semibold uppercase tracking-wide text-app-text-muted">
              {selectedLapA?.ownership === "others" ? m.import_ownership_others() : m.import_ownership_mine()}
            </span>
          )}
        </div>
      </div>
      ...
      <div className="flex w-full min-w-0 flex-col gap-1 @sm/workspace:w-auto @sm/workspace:min-w-[120px] @sm/workspace:flex-1 @3xl/workspace:max-w-[200px]">
        ...
        <div className="flex items-center gap-2">
          ...
          {lapBId != null && (
            <span className="shrink-0 rounded border border-app-border px-1.5 py-0.5 text-xs font-semibold uppercase tracking-wide text-app-text-muted">
              {selectedLapB?.ownership === "others" ? m.import_ownership_others() : m.import_ownership_mine()}
            </span>
          )}
        </div>
      </div>
