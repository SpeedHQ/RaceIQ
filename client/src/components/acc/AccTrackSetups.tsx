          ...
          {filteredSetups.map((s, i) => (
            ...
            >
              ...
              <div className="flex items-center gap-0.5 shrink-0 justify-center">
                {s.hasRace && (
                  <span className="setup-variant-badge text-[8px] px-1 py-0.5 rounded font-bold" data-setup-variant="race" title={m.accsetup_setup_type_race_title()}>
                    R
                  </span>
                )}
                {s.hasQuali && (
                  <span className="setup-variant-badge text-[8px] px-1 py-0.5 rounded font-bold" data-setup-variant="qualifying" title={m.accsetup_setup_type_qualifying_title()}>
                    Q
                  </span>
                )}
                {s.hasWet && (
                  <span className="setup-variant-badge text-[8px] px-1 py-0.5 rounded font-bold" data-setup-variant="wet" title={m.accsetup_setup_type_wet_title()}>
                    W
                  </span>
                )}
                {s.videoUrl && (
                  <span className="text-[9px] ml-0.5" style={{ color: "var(--brand-provider-youtube)" }} title={m.accsetup_setup_type_has_video_title()}>
                    ▶
                  </span>
                )}
                {(s.downloadUrl || s.setupFile) && (
                  <span className="setup-variant-badge text-[8px] px-1 py-0.5 rounded font-bold" data-setup-variant="file" title={m.accsetup_setup_type_has_file_title()}>
                    {m.accsetup_file()}
                  </span>
                )}
              </div>
              <span className="text-app-unit font-mono text-(--lap-pace-on-target) shrink-0 w-16 text-right">{s.lapTime || "—"}</span>
            </div>
          ))}
        </div>
      </div>
          ...
          <div className="flex-1 min-w-0 overflow-y-auto space-y-3">
            ...
            {(setup.hasRace || setup.hasQuali || setup.hasSafe || setup.hasWet) && (
              <div className="flex items-center gap-1.5 flex-wrap">
                {setup.hasRace && <span className="setup-variant-badge text-[10px] px-2 py-0.5 rounded-full border font-medium" data-setup-variant="race">{m.label_race()}</span>}
                {setup.hasQuali && <span className="setup-variant-badge text-[10px] px-2 py-0.5 rounded-full border font-medium" data-setup-variant="qualifying">{m.accsetup_badge_qualify()}</span>}
                {setup.hasSafe && <span className="setup-variant-badge text-[10px] px-2 py-0.5 rounded-full border font-medium" data-setup-variant="safe">{m.accsetup_badge_safe()}</span>}
                {setup.hasWet && <span className="setup-variant-badge text-[10px] px-2 py-0.5 rounded-full border font-medium" data-setup-variant="wet">{m.accsetup_badge_wet()}</span>}
              </div>
            )}
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {(() => {
                ...
                return (
                  <a
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 text-app-unit font-semibold rounded transition-colors ${isVideo ? "provider-badge hover:opacity-80" : "bg-app-accent/15 text-app-accent hover:bg-app-accent/25"}`}
              ...
              {setup.setupFile && (
                <button
                  ...
                  className="px-3 py-1.5 text-app-unit font-semibold bg-status-success/15 text-status-success rounded hover:bg-status-success/25 transition-colors disabled:opacity-50"
                >
              ...
              {setup.videoUrl && setup.videoUrl !== setup.downloadUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="provider-badge flex items-center gap-1.5 px-3 py-1.5 text-app-unit font-semibold rounded hover:opacity-80 transition-colors"
