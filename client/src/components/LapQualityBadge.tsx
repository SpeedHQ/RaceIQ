...
type DiagnosticReason = Pick<EligibilityReason, "code" | "timeRange" | "distanceRange" | "semanticIds" | "evidenceIds"> & { eventIds: string[] };
...
export function LapQualityBadge({ lap, policyId = "corner-trace", size = "compact", decisionOverride }: LapQualityBadgeProps) {
  ...
  const rebuild = useMutation({
    ...
    onSuccess: async () => {
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: ["session-quality", sessionId] }),
  ...
  const decisions = decisionOverride === undefined ? persistedDecisions : [...persistedDecisions.filter((item) => item.policyId !== policyId), ...(decisionOverride ? [decisionOverride] : [])];
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent size="md" layout="scrollable">
          ...
          {reasons.length === 0 ? (
            ...
            <ul className="space-y-2 text-app-caption text-app-text-muted">
              {reasons.map((reason) => {
                ...
                return (
                  <li key={reason.code} className="rounded-md border border-app-border px-3 py-2">
        ...
        {(rebuildAction === "unavailable" || rebuild.isError) && (
          <p className="text-app-caption text-status-danger">{rebuildAction === "unavailable" ? m.quality_rebuild_unavailable() : m.quality_rebuild_failed()}</p>
        )}
