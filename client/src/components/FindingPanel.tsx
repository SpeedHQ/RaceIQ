  ...
  return narratives
    .filter((narrative) => {
      ...
      return candidate != null
        ...
        && candidate.findingIds.includes(findingId)
...
function EvidenceNavigation({ evidence, onJumpToFrame }: { evidence: TelemetryRangeFindingEvidence[]; onJumpToFrame: (frameIndex: number) => void }) {
...
function FindingCard({
  ...
  const matchingNarratives = narrativesForFinding(narratives, finding.id);
  ...
  return (
    <article className={`space-y-3 rounded border bg-app-surface-alt p-3 ${findingBorder(finding)}`} data-finding-id={finding.id} data-status={finding.status} data-confidence={finding.confidence}>
      ...
      <EvidenceNavigation evidence={telemetryEvidence} onJumpToFrame={onJumpToFrame} />
...
export function FindingPanel({
  ...
  return (
    <div className="flex flex-col gap-3" aria-label="Deterministic findings">
      {ordered.map((finding) => (
        <FindingCard key={finding.id} finding={finding} narratives={narratives} onJumpToFrame={onJumpToFrame} />
      ))}
    </div>
  );
}
