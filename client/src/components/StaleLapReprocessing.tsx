...
function ReprocessStatusIcon({ status }: { status: "submitting" | "progressing" | "success" | "error" }) {
  if (status === "success") {
    return <CheckCircle2 aria-hidden="true" className="size-5 shrink-0 text-green-400" />;
  }
  if (status === "error") {
    return <TriangleAlert aria-hidden="true" className="size-5 shrink-0 text-red-400" />;
  }
  return <RefreshCw aria-hidden="true" className="size-5 shrink-0 animate-spin text-blue-400" />;
}
...
export function StaleLapReprocessing() {
  ...
  return (
    <>
      {showNotification && (
        <div role="status" className="fixed right-4 bottom-4 z-50 w-72 rounded-lg border border-blue-500/30 bg-app-surface p-4 shadow-xl">
          <div className="mb-2 flex items-center gap-2">
            <RefreshCw aria-hidden="true" className="size-4 shrink-0 text-blue-400" />
        ...
        <Dialog open={reprocessState.open} onOpenChange={(open) => !open && dismissReprocess()}>
          <DialogContent showCloseButton={false} className="w-96 border border-app-border bg-app-surface p-6 text-app-text sm:max-w-96">
            <DialogHeader>
              ...
              <DialogDescription className={reprocessState.status === "error" ? "text-red-300" : "text-app-text-muted"}>
                {reprocessState.status === "error"
            ...
            {reprocessState.status !== "error" && (
              ...
              >
                <div className={`h-full rounded-full transition-all duration-300 ${reprocessState.status === "success" ? "bg-green-500" : "bg-blue-500"}`} style={{ width: `${percent}%` }} />
              </div>
            )}

            {reprocessState.status === "success" && <p className="text-center text-xs text-green-400">{m.root_all_sessions_updated()}</p>}
