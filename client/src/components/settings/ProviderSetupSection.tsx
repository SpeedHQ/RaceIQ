          ...
          return (
            <div key={provider} className="px-3 py-3">
              <div className="flex items-center justify-between gap-3">
                <span className="flex items-center gap-2"><span className="text-sm font-medium text-app-text">{info.label.replace(" API Key", "")}</span>{stored && <span className="rounded border border-status-success/40 px-1.5 py-0.5 text-app-caption uppercase tracking-wider text-status-success">{m.ai_configured()}</span>}</span>
