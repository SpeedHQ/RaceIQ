          ...
          return (
            <Button
              ...
              className={`w-full flex items-center justify-between rounded-lg border px-4 py-3 text-left transition-all ${
                visible ? "border-app-accent/30 bg-app-accent/5" : "border-app-border bg-app-surface-alt hover:border-app-border-hover"
              }`}
            >
              ...
              <span className={`relative inline-flex h-5 w-9 shrink-0 items-center rounded-full transition-colors ${visible ? "bg-app-accent" : "bg-app-border"}`}>
                <span className={`inline-block h-3.5 w-3.5 rounded-full bg-app-text shadow transition-transform ${visible ? "translate-x-[18px]" : "translate-x-[3px]"}`} />
              </span>
            </Button>
          );
        })}
