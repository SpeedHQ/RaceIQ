...
export function AiProviderPicker({ id, value, onChange, providers, keyStatus }: { id: string; value: string; onChange: (value: string) => void; providers?: Provider[]; keyStatus: Record<string, boolean> }) {
  return <SearchSelect id={id} value={value} onChange={onChange} options={[{ value: "", label: m.ai_provider_none() }, ...(providers ?? []).map((provider) => ({ value: provider.id, label: provider.name, disabled: !keyStatus[provider.id] }))]} ariaLabel={m.ai_provider_label()} />;
}
