      ...
      <div className="text-app-compact font-mono space-y-1.5 mb-3">
        ...
        <div>
          <div className="flex justify-between mb-0.5">
            ...
            <span className="tabular-nums text-(--telemetry-ers-store)">{pct(store)}</span>
          </div>
          <div className="h-1.5 bg-app-surface-alt rounded-full overflow-hidden">
            <div className={`h-full rounded-full transition-all ${ersBarColor}`} style={{ width: `${Math.max(0, Math.min(100, ersPct))}%` }} />
          </div>
        </div>
        <div className="flex justify-between">
          <span className="text-app-text-muted">{m.analyse_deployed()}</span>
          <span className="tabular-nums text-(--telemetry-ers-deployed)">{pct(deployed)}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-app-text-muted">{m.analyse_harvested()}</span>
          <span className="tabular-nums text-(--severity-nominal)">{pct(harvested)}</span>
        </div>
        <div className="flex justify-between">
