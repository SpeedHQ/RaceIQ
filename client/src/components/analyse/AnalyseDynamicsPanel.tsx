...
export function AnalyseDynamicsPanel({ frame, gameId, units }: Props) {
  ...
  const slipAngles = slipAngleBinding && slipAngleId && frame.states[slipAngleId] !== "missing" ? resolveWheelMetric(frame, slipAngleBinding) : [null, null, null, null];
  const rawSlipRatios = frame.values["tires.tire-slip-ratio"];
  const slipRatioState = frame.states["tires.tire-slip-ratio"];
  const slipRatioAvailable =
    Array.isArray(rawSlipRatios) &&
  ...
  const slipRatios = slipRatioMetric.source === "unavailable" || !slipRatioAvailable ? [null, null, null, null] : states.map((state) => state?.slipRatio ?? null);
  ...
  const surfaceText =
    surfaceValue === -1
      ? m.analyse_surface_not_in_world()
      : surfaceValue === 0
        ? m.analyse_surface_off_track()
        : surfaceValue === 1
          ? m.analyse_surface_pit_stall()
          : surfaceValue === 2
            ? m.analyse_surface_approaching_pits()
            : surfaceValue === 3
              ? m.analyse_surface_on_track()
              : m.analyse_surface_unknown();
  ...
  const balanceTooltip = balance && (
    <span className="pointer-events-none absolute top-full left-0 z-50 mt-2 hidden w-[min(320px,calc(100vw-2rem))] rounded border border-app-border-input bg-app-surface-alt px-2.5 py-2 text-app-caption text-app-text-secondary normal-case tracking-normal group-hover:block group-focus-within:block">
      <span className="block mb-1">
        {balance.slipAvailable ? "Yaw rate vs path curvature + front/rear slip-angle delta." : "Yaw rate versus path curvature (tire slip angles unavailable for this game)."}
      </span>
      <span className="block mb-2 text-app-text-dim">
        {balance.slipAvailable ? <>+ = understeer (front slip &gt; rear) | − = oversteer (body yawing past Ay/V)</> : <>+ = understeer (under-rotating) | − = oversteer (over-rotating)</>}
        <br />
      ...
      <svg viewBox="0 0 200 110" className="w-full h-auto" aria-hidden="true">
        {[
          {
            ...
            y: 16,
            desc: balance.slipAvailable ? `F ${balance.frontSlipDeg.toFixed(1)}° / R ${balance.rearSlipDeg.toFixed(1)}°` : "Unavailable",
            opacity: balance.slipAvailable ? 1 : 0.35,
          },
          {
            label: "Yaw",
            value: balance.uYaw,
            color: signedBalanceColor(balance.uYaw, 0.05),
            y: 40,
            desc: `err ${balance.yawError > 0 ? "+" : ""}${balance.yawError.toFixed(2)} r/s (path ${balance.yawRatePath.toFixed(2)})`,
            opacity: Math.min(1, balance.yawRatePath / 0.15),
          },
        ].map((signal) => {
          ...
          return (
            <g key={signal.label} opacity={signal.opacity}>
              <text x="0" y={signal.y - 4} fill="currentColor" opacity="0.5" fontSize="6.5">
                {signal.label}
              </text>
              ...
              <text x="0" y={signal.y + 20} fill="currentColor" opacity="0.35" fontSize="6">
                {signal.desc}
              </text>
            </g>
          );
        ...
        <text
          ...
          y="70"
          textAnchor="middle"
          fill={balance.slipAvailable ? (balance.signalsAgree ? "var(--status-success)" : "var(--status-warning)") : "var(--status-warning)"}
          fontSize="7"
          fontWeight="var(--font-weight-semibold)"
        >
          {balance.slipAvailable ? (balance.signalsAgree ? "SIGNALS AGREE — blended 50/50" : "CONFLICT — slip angle used alone") : "YAW ONLY — curvature signal"}
        </text>
        ...
        <text x="20" y="106" textAnchor="middle" fill="var(--balance-negative)" fontSize="7" fontWeight="var(--font-weight-semibold)">
          {m.dynamics_over()}
        </text>
        <text x="100" y="106" textAnchor="middle" fill="var(--balance-neutral)" fontSize="7" fontWeight="var(--font-weight-semibold)">
          {m.dynamics_neutral()}
        </text>
        <text x="180" y="106" textAnchor="middle" fill="var(--balance-positive)" fontSize="7" fontWeight="var(--font-weight-semibold)">
          {m.dynamics_under()}
        </text>
      </svg>
    </span>
  );
  const surfaceCell = (index: number) =>
    bool(frame, "tires.wheel-on-rumble-strip", index)
  ...
  const tractionCell = (index: number) => {
    ...
    return state && angle != null && ratio != null
      ? (() => {
          const result = tireState(state.state, ratio, angle);
          return C(result.label, result.color);
        })()
      : unavailable;
  };
    ...
    <div className="text-app-compact font-mono space-y-1.5 mb-3">
      <div className="flex justify-between">
        <Button
          ...
          size="content"
          type="button"
          className="group relative flex items-center gap-1 text-app-text-muted outline-none focus-visible:ring-2 focus-visible:ring-app-accent"
          aria-label={`${m.label_balance()}: Balance tooltip`}
        >
          {m.label_balance()}{" "}
          {balance && (
            <>
              <Info className="size-3 cursor-help text-app-text-dim" aria-hidden="true" />
              {balanceTooltip}
            </>
          )}
        </Button>
        <span className="tabular-nums text-app-text-dim">
          {balance ? (
            <span style={{ color: balanceColor }}>
              {balance.state === "neutral" ? "Neutral" : balance.state === "understeer" ? "Understeer" : "Oversteer"}({balance.balance > 0 ? "+" : ""}
      ...
      <div className="flex justify-between">
        ...
        <span className="tabular-nums text-app-text">{brakeBias == null ? unavailable : `${(brakeBias * 100).toFixed(1)}%F`}</span>
      </div>
      <WheelTable
        rows={[
          wheelRow(m.analyse_dynamics_grip_ask(), (i) => (grip[i] == null ? unavailable : C(`${(grip[i]! * 100).toFixed(0)}%`, frictionUtilColor(grip[i]!)))),
          wheelRow(m.analyse_dynamics_traction(), tractionCell),
          wheelRow(analysis.tireTemperature.source === "direct" && analysis.tireTemperature.freshness === "pit-snapshot" ? m.analyse_wheels_pit_temp() : m.analyse_dynamics_temp(), (i) =>
            temps[i] == null ? unavailable : C(tireTempLabel(units.toTempC(temps[i]!), units.thresholds).label, tireTempLabel(units.toTempC(temps[i]!), units.thresholds).color),
          ),
          ...(surfaceBinding?.kind === "group" ? [wheelRow(m.analyse_dynamics_surface(), surfaceCell)] : []),
        ]}
      />
      {surfaceBinding?.kind === "value" && (
        <div className="flex justify-between">
          <span className="text-app-text-muted">{m.analyse_dynamics_surface()}</span>
          <span className="text-app-text">{surfaceText}</span>
        </div>
      )}
      <WheelTable
        title={
          <span className="flex items-center gap-1 group relative">
            {m.label_slip()}
            <Info className="size-3 text-app-text-dim cursor-help" aria-hidden="true" />
          </span>
        }
        borderTop
        rows={[
          wheelRow(m.analyse_dynamics_ratio(), (i) => (slipRatios[i] == null ? unavailable : C(`${(slipRatios[i]! * 100).toFixed(0)}%`, slipRatioColor(slipRatios[i]!)))),
          wheelRow(m.analyse_dynamics_angle(), (i) => (slipAngles[i] == null ? unavailable : C(`${((slipAngles[i]! * 180) / Math.PI).toFixed(1)}°`, angleColor(slipAngles[i]!)))),
        ]}
      />
    </div>
  );
