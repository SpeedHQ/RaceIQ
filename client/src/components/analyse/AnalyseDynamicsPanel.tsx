...
export function AnalyseDynamicsPanel({ frame, gameId, units }: Props) {
  ...
  return <div className="text-app-compact font-mono space-y-1.5 mb-3">
    ...
    <WheelTable title={m.label_slip()} borderTop rows={[
      { label: m.analyse_dynamics_ratio(), fl: C(`${((slipRatios[0] ?? 0) * 100).toFixed(0)}%`, slipRatioColor(slipRatios[0] ?? 0)), fr: C(`${((slipRatios[1] ?? 0) * 100).toFixed(0)}%`, slipRatioColor(slipRatios[1] ?? 0)), rl: C(`${((slipRatios[2] ?? 0) * 100).toFixed(0)}%`, slipRatioColor(slipRatios[2] ?? 0)), rr: C(`${((slipRatios[3] ?? 0) * 100).toFixed(0)}%`, slipRatioColor(slipRatios[3] ?? 0)) },
