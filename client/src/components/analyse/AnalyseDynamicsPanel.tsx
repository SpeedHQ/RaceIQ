...
export function AnalyseDynamicsPanel({ frame, gameId, units }: Props) {
  ...
  return <div className="text-app-compact font-mono space-y-1.5 mb-3">
    ...
    <WheelTable rows={[wheelRow(m.analyse_dynamics_grip_ask(), (i) => grip[i] == null ? unavailable : C(`${(grip[i]! * 100).toFixed(0)}%`, frictionUtilColor(grip[i]!))), wheelRow(m.analyse_dynamics_traction(), tractionCell), wheelRow(coreTemps.some((value) => value != null) ? m.label_surface() : (analysis.tireTemperature.source === "direct" && analysis.tireTemperature.freshness === "pit-snapshot" ? m.analyse_wheels_pit_temp() : m.analyse_dynamics_temp()), (i) => temps[i] == null ? unavailable : C(tireTempLabel(units.toTempC(temps[i]!), units.thresholds).label, tireTempLabel(units.toTempC(temps[i]!), units.thresholds).color)), ...(coreTemps.some((value) => value != null) ? [wheelRow(m.label_core(), (i) => coreTemps[i] == null ? unavailable : C(tireTempLabel(units.toTempC(coreTemps[i]!), units.thresholds).label, tireTempLabel(units.toTempC(coreTemps[i]!), units.thresholds).color))] : []), ...(surfaceBinding?.kind === "group" ? [wheelRow(m.analyse_dynamics_surface(), surfaceCell)] : [])]} />
