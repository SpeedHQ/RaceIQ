...
export function buildAnalyseClipboardText({ frame, gameId, units }: { frame: SemanticAnalysisFrame; gameId: GameId; units: ReturnType<typeof useUnits> }): string {
  ...
  const displayTemperature = (temperatureC: number | null) => (temperatureC == null ? "Unavailable" : units.temp(temperatureC).toFixed(0));
  ...
  lines.push(
    ...
    `FL: ${displayTemperature(temp[0])}  FR: ${displayTemperature(temp[1])}`,
    `RL: ${displayTemperature(temp[2])}  RR: ${displayTemperature(temp[3])}`,
  );
  lines.push(
    "",
    ...
    >
      <TabsList variant="underline" className="w-full shrink-0">
        ...
        <TabsTrigger value="insights" className="flex-1">
          Findings{findings.length > 0 && <span className="ml-1 rounded-full bg-app-border-input px-1.5 text-app-micro text-app-text">{findings.length}</span>}
        </TabsTrigger>
      </TabsList>
      ...
      <TabsContent value="insights" className="min-h-0 flex-1 overflow-y-auto p-3">
        {sidebarTab === "insights" && <FindingPanel findings={findings} narratives={narratives} receipt={findingReceipt} pending={findingPending} onEvidenceSelect={onEvidenceSelect} />}
      </TabsContent>
    </Tabs>
  );
}
