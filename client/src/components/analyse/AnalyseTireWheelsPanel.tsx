...
export function AnalyseTireWheelsPanel({ frame, gameId, units, wearRate }: Props) {
  ...
  const tempCell = (value: number | null) =>
    value == null ? unavailable : <span style={{ color: tireTempColor(units.toTempC(value), units.thresholds) }}>{`${units.temp(value).toFixed(0)}${units.tempLabel}`}</span>;
  ...
  const rows = [
    {
      ...
      fl: speed[0]?.toFixed(1) ?? unavailable,
      fr: speed[1]?.toFixed(1) ?? unavailable,
      rl: speed[2]?.toFixed(1) ?? unavailable,
      rr: speed[3]?.toFixed(1) ?? unavailable,
    },
    { label: pitTemperature ? m.analyse_wheels_pit_temp() : m.analyse_wheels_temp(), fl: tempCell(temp[0]), fr: tempCell(temp[1]), rl: tempCell(temp[2]), rr: tempCell(temp[3]) },
    {
      ...
      fl: health[0] == null ? unavailable : <span style={{ color: tireHealthColor(health[0], hThresholds) }}>{`${((1 - health[0]) * 100).toFixed(1)}%`}</span>,
      fr: health[1] == null ? unavailable : <span style={{ color: tireHealthColor(health[1], hThresholds) }}>{`${((1 - health[1]) * 100).toFixed(1)}%`}</span>,
      rl: health[2] == null ? unavailable : <span style={{ color: tireHealthColor(health[2], hThresholds) }}>{`${((1 - health[2]) * 100).toFixed(1)}%`}</span>,
      rr: health[3] == null ? unavailable : <span style={{ color: tireHealthColor(health[3], hThresholds) }}>{`${((1 - health[3]) * 100).toFixed(1)}%`}</span>,
    },
    ...(analysis.tireWearRate.source !== "unavailable"
      ? [
          {
            label: m.analyse_wheels_wear_s(),
            fl: <span style={{ color: wearRateColor(wearRate ? wearRate.FL * 100 : null) }}>{wearRate ? `${(wearRate.FL * 100).toFixed(3)}%` : "—"}</span>,
            fr: <span style={{ color: wearRateColor(wearRate ? wearRate.FR * 100 : null) }}>{wearRate ? `${(wearRate.FR * 100).toFixed(3)}%` : "—"}</span>,
            rl: <span style={{ color: wearRateColor(wearRate ? wearRate.RL * 100 : null) }}>{wearRate ? `${(wearRate.RL * 100).toFixed(3)}%` : "—"}</span>,
            rr: <span style={{ color: wearRateColor(wearRate ? wearRate.RR * 100 : null) }}>{wearRate ? `${(wearRate.RR * 100).toFixed(3)}%` : "—"}</span>,
          },
        ]
      : []),
    ...((brake[0] ?? 0) > 0 || (brake[1] ?? 0) > 0
      ? [
          {
            label: m.analyse_wheels_brake(),
            fl: brake[0] == null ? unavailable : <span style={{ color: brakeTempColor(brake[0], false) }}>{`${brake[0].toFixed(0)}°C`}</span>,
            fr: brake[1] == null ? unavailable : <span style={{ color: brakeTempColor(brake[1], false) }}>{`${brake[1].toFixed(0)}°C`}</span>,
            rl: brake[2] == null ? unavailable : <span style={{ color: brakeTempColor(brake[2], true) }}>{`${brake[2].toFixed(0)}°C`}</span>,
            rr: brake[3] == null ? unavailable : <span style={{ color: brakeTempColor(brake[3], true) }}>{`${brake[3].toFixed(0)}°C`}</span>,
          },
        ]
      : []),
      ...
      ? [
          {
            ...
            fl: pressure[0] == null ? unavailable : <span style={{ color: coldPressure ? "var(--app-text)" : tirePressureColor(pressure[0], optimal) }}>{`${pressure[0].toFixed(1)} psi`}</span>,
            fr: pressure[1] == null ? unavailable : <span style={{ color: coldPressure ? "var(--app-text)" : tirePressureColor(pressure[1], optimal) }}>{`${pressure[1].toFixed(1)} psi`}</span>,
            rl: pressure[2] == null ? unavailable : <span style={{ color: coldPressure ? "var(--app-text)" : tirePressureColor(pressure[2], optimal) }}>{`${pressure[2].toFixed(1)} psi`}</span>,
            rr: pressure[3] == null ? unavailable : <span style={{ color: coldPressure ? "var(--app-text)" : tirePressureColor(pressure[3], optimal) }}>{`${pressure[3].toFixed(1)} psi`}</span>,
          },
        ]
      : []),
  ];
