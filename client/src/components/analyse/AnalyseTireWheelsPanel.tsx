...
export function AnalyseTireWheelsPanel({ frame, gameId, units, wearRate }: Props) {
  ...
  const carcassTemp = semanticWheelNumbers(frame, "tire.temperature.carcass.representative");
  ...
  const dualTemperature = analysis.tireTemperature.source !== "unavailable" && analysis.tireTemperature.binding?.kind === "value" && analysis.tireTemperature.binding.semanticId === "tire.temperature.surface.representative" && temp.some((value) => value != null) && (coreTemp.some((value) => value != null) || carcassTemp.some((value) => value != null));
  ...
  const tempCell = (value: number | null, wheel: string, label: string) => {
    const text = value == null ? m.analyse_unavailable() : `${units.temp(value).toFixed(0)}${units.tempLabel}`;
    return <span title={`${wheel} ${label}: ${text}`} aria-label={`${wheel} ${label}: ${text}`} style={value == null ? undefined : { color: tireTempColor(value, units.thresholds) }}>{value == null ? unavailable : text}</span>;
  };
  const row = (label: string, values: readonly (number | null)[]) => ({ label, fl: tempCell(values[0] ?? null, "FL", label), fr: tempCell(values[1] ?? null, "FR", label), rl: tempCell(values[2] ?? null, "RL", label), rr: tempCell(values[3] ?? null, "RR", label) });
  const interiorRows = [...(carcassTemp.some((value) => value != null) ? [row(m.label_carcass(), carcassTemp)] : []), ...(coreTemp.some((value) => value != null) ? [row(m.label_core(), coreTemp)] : [])];
  const rows = [
    { label: m.analyse_wheels_rotation_s(), fl: speed[0] == null ? unavailable : Math.abs(speed[0]).toFixed(1), fr: speed[1] == null ? unavailable : Math.abs(speed[1]).toFixed(1), rl: speed[2] == null ? unavailable : Math.abs(speed[2]).toFixed(1), rr: speed[3] == null ? unavailable : Math.abs(speed[3]).toFixed(1) },
    ...(profile ? [...(temp.some((value) => value != null) ? [row(m.label_surface(), temp)] : []), row(m.label_inner(), semanticWheelNumbers(frame, "tire.temperature.surface.inner")), row(m.label_middle(), semanticWheelNumbers(frame, "tire.temperature.surface.middle")), row(m.label_outer(), semanticWheelNumbers(frame, "tire.temperature.surface.outer")), ...interiorRows] : [row(dualTemperature ? m.label_surface() : (pitTemperature ? m.analyse_wheels_pit_temp() : m.analyse_wheels_temp()), temp), ...interiorRows]),
