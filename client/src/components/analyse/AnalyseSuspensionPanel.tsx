...
export function AnalyseSuspensionPanel({ frame, gameId }: Props) {
  ...
  const normalizedValues = analysis.suspensionTravel.source !== "unavailable" && analysis.suspensionTravel.binding?.kind === "value" && analysis.suspensionTravel.display === "normalized" ? resolveWheelMetric(frame, analysis.suspensionTravel.binding) : [null, null, null, null];
