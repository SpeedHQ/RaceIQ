...
export function AnalyseSuspensionPanel({ frame, gameId }: Props) {
  ...
  const millimeterValues =
    analysis.suspensionTravel.source !== "unavailable" && analysis.suspensionTravel.binding?.kind === "value" && analysis.suspensionTravel.display === "millimeters"
      ? resolveWheelMetric(frame, analysis.suspensionTravel.binding).map((value) => (value == null ? null : value * 1000))
      : [null, null, null, null];
  ...
  const bias =
    analysis.suspensionCompressionBias.source !== "unavailable" && normalized.every((value): value is number => value != null)
  ...
  const cell = (index: number) =>
    showMillimeters ? (
      millimeters[index] == null ? (
        unavailable
      ) : (
        `${Math.round(millimeters[index]!)}mm`
      )
    ) : normalized[index] == null ? (
      unavailable
    ) : (
      <span style={{ color: operatingRangeColor(normalized[index]!, [0.25, 0.65, 0.85]) }}>{`${(normalized[index]! * 100).toFixed(0)}%`}</span>
    );
  return (
    <WheelTable
      title={title}
      borderTop
      rows={[
        { label: m.dataguide_travel(), fl: cell(0), fr: cell(1), rl: cell(2), rr: cell(3) },
        {
          label: m.analyse_suspension_compression_bias(),
          fl: bias == null ? unavailable : `${m.analyse_suspension_front()} ${(bias.front * 100).toFixed(0)}%`,
          rl: bias == null ? unavailable : `${m.analyse_suspension_left()} ${(bias.left * 100).toFixed(0)}%`,
          fr: "",
          rr: "",
          span2: true,
        },
