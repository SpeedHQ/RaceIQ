...
export function SetupList({
  ...
  return (
    <div className="space-y-2">
      {!hasTune && <p className="text-[10px] text-ai-accent/70 leading-snug">{m.aidisplay_no_tune_linked()}</p>}
      {setup.map((item) => {
          ...
          <div className="space-y-1.5">
            {analysis.corners.map((corner) => (
              ...
              >
                ...
                <p className="text-[10px] text-(--delta-gain)/80 mt-0.5">{corner.fix}</p>
              </TrackCard>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-1.5">
            {analysis.coaching.map((item, i) => (
              <TrackCard key={item.tip} seg={findSegment(lookupSegs, item.tip, item.detail)} color="warning" onJumpToFrac={onJumpToFrac} onHighlightsChange={onHighlightsChange} className="flex gap-2">
                <span className="text-ai-accent/60 text-[10px] font-mono mt-0.5">
                  {i + 1}.
                </span>
                <div>
        ...
        <div className="flex items-center gap-1.5 pt-1.5 border-t border-app-border-input/30">
          ...
          {onExport && (
            <button
              type="button"
              onClick={onExport}
              className="flex items-center gap-1 text-[9px] text-app-text-muted hover:text-app-text px-1.5 py-0.5 rounded border border-transparent hover:border-app-border-hover transition-colors"
          ...
          {onRegenerate && (
            <button
              ...
              disabled={loading}
              className="flex items-center gap-1 text-[9px] text-app-text-muted hover:text-app-text px-1.5 py-0.5 rounded border border-transparent hover:border-app-border-hover transition-colors disabled:opacity-50"
          ...
          {onClear && (
            <button
              type="button"
              onClick={onClear}
              className="flex items-center gap-1 text-[9px] text-app-text-muted hover:text-status-danger px-1.5 py-0.5 rounded border border-transparent hover:border-app-border-hover transition-colors"
