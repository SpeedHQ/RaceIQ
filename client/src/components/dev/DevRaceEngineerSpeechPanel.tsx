...
const scenarioStates = [
  { id: "cross-finish", label: "Cross finish line", detail: "Lap completed", priority: "normal", color: "text-app-signal-cyan" },
  { id: "lap-invalidated", label: "Lap invalidated", detail: "Lap time rejected", priority: "critical", color: "text-app-signal-amber" },
  { id: "damage-detected", label: "Damage detected", detail: "Car condition changed", priority: "critical", color: "text-app-signal-red" },
  { id: "opponent-ahead", label: "Opponent ahead", detail: "Car in front within gap", priority: "high", color: "text-app-signal-amber" },
...
export function DevRaceEngineerSpeechPanel() {
  ...
  const [activeScenarioStates, setActiveScenarioStates] = useState<ScenarioState[]>(["cross-finish"]);
  const [scenarioOutput, setScenarioOutput] = useState("No voice trigger selected.");
  const [scenarioDecision, setScenarioDecision] = useState("Waiting for trigger");
  ...
  const renderPace = async (nextRelation: (typeof relations)[number], scope: "class" | "overall" = "class"): Promise<RenderedPace> => {
    const response = await fetch("/api/dev/live-engineer/preview", {
      method: "POST",
      ...
      body: JSON.stringify({
        ...
        scope,
        playerLapNumber: 1,
        playerLapTimeMs: 60_000 + sampleDeltaMs,
        benchmarkLapTimeMs: 60_000,
  ...
  const testScenario = async () => {
    ...
    setScenarioDecision(
      special
        ? `${scenarioStates.find((item) => item.id === special.state)?.label} wins priority`
        : side
          ? `Three-wide ${side} wins priority over lap boundary`
          : activeScenarioStates.includes("cross-finish")
    ...
    <div className="h-full overflow-y-auto p-6">
      ...
      <section className="mt-6 rounded border border-app-border p-4">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h3 className="font-semibold">Scenario state machine</h3>
            <p className="mt-1 text-xs text-app-text-muted">Activate multiple telemetry states, then trigger voice to see arbitration and output.</p>
          </div>
          <span
            className={`rounded-full border px-2 py-1 text-xs font-semibold uppercase tracking-wide ${engineBusy ? "border-app-signal-amber text-app-signal-amber" : "border-app-signal-cyan text-app-signal-cyan"}`}
          >
            {engineBusy ? "Evaluating" : "Engine online"}
          </span>
        </div>
        <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_22rem]">
          <div>
            <div className="grid gap-2 sm:grid-cols-3">
              {scenarioStates.map((item) => (
                <button
                  key={item.id}
                  type="button"
                  onClick={() => toggleScenarioState(item.id)}
                  className={`rounded border px-3 py-3 text-left transition-colors ${activeScenarioStates.includes(item.id) ? "border-app-accent bg-app-accent/15" : "border-app-border bg-app-surface-alt hover:border-app-accent/60"}`}
                  aria-pressed={activeScenarioStates.includes(item.id)}
                >
                  ...
                  <span className="mt-2 block text-xs uppercase tracking-wide text-app-text-muted">
                    {item.priority} priority · {activeScenarioStates.includes(item.id) ? "active" : "inactive"}
                  </span>
                </button>
              ))}
            </div>
            <div className="mt-4 flex flex-wrap items-center gap-3">
              <select aria-label="Pace candidate" value={engineState} onChange={(event) => setEngineState(event.target.value as (typeof relations)[number])}>
                {relations.map((item) => (
                  <option key={item}>{item}</option>
                ))}
              </select>
              <Button onClick={() => void testScenario()} disabled={engineBusy || audio.catalog === null}>
                Trigger voice
              </Button>
              <Button variant="outline" onClick={() => void testState(engineState)} disabled={engineBusy || audio.catalog === null}>
                Test pace only
              </Button>
            ...
            <div className="mt-4 grid gap-2 sm:grid-cols-4">
              <div className="rounded border border-app-border bg-app-surface-alt p-3">
                <div className="text-xs uppercase tracking-wide text-app-text-muted">Active inputs</div>
                <div className="mt-1 font-semibold">{activeScenarioStates.length || "None"}</div>
              </div>
              <div className="rounded border border-app-accent/50 bg-app-accent/5 p-4 sm:col-span-2">
                <div className="text-xs uppercase tracking-wide text-app-text-muted">Decision</div>
                <div className="mt-2 min-h-12 text-lg font-semibold leading-tight">{scenarioDecision}</div>
              </div>
              <div className="rounded border border-app-border bg-app-surface-alt p-3">
                <div className="text-xs uppercase tracking-wide text-app-text-muted">Pace priority</div>
                <div className={`mt-1 font-semibold ${selectedDefinition.color}`}>{selectedDefinition.priority}</div>
              </div>
              <div className="rounded border border-app-border bg-app-surface-alt p-3">
                <div className="text-xs uppercase tracking-wide text-app-text-muted">Trace</div>
                <div className="mt-1 font-semibold">{engineEvents.length} events</div>
              </div>
            </div>
          </div>
          <aside aria-label="Scenario output and logic diagram" className="rounded border border-app-border bg-app-surface-alt p-3">
            <h4 className="text-xs font-semibold uppercase tracking-wide text-app-text-muted">Would say</h4>
            <div className="mt-2 min-h-16 rounded border border-app-accent/50 bg-app-accent/5 p-3 text-sm font-semibold">{scenarioOutput}</div>
            <h4 className="mt-4 text-xs font-semibold uppercase tracking-wide text-app-text-muted">Arbitration path</h4>
            <div className="mt-2 space-y-1 text-xs">
              <div className={`rounded border px-2 py-2 ${activeScenarioStates.includes("cross-finish") ? "border-app-accent bg-app-accent/15" : "border-app-border"}`}>
                Cross finish → lap-time candidate
              </div>
              <div className="pl-3 text-app-text-muted">↓</div>
              <div className={`rounded border px-2 py-2 ${activeScenarioStates.some((item) => item.startsWith("three-wide")) ? "border-app-signal-red bg-app-signal-red/10" : "border-app-border"}`}>
                Three-wide → spotter candidate · high priority
              </div>
              <div className="pl-3 text-app-text-muted">↓</div>
              <div className="rounded border border-app-border px-2 py-2">Priority arbitration → voice line</div>
            </div>
          </aside>
        </div>
        <div className="mt-4 border-t border-app-border pt-3">
          <div className="flex items-center justify-between text-xs uppercase tracking-wide text-app-text-muted">
            <span>Pace state candidates</span>
            <span>click to select fallback</span>
          </div>
          <div className="mt-2 grid gap-2 sm:grid-cols-5">
            {stateDefinitions.map((item) => (
              <button
                ...
                type="button"
                onClick={() => {
                  setEngineState(item.id);
                  setRelation(item.id);
                }}
                className={`rounded border px-2 py-2 text-left text-xs ${engineState === item.id ? "border-app-accent bg-app-accent/15" : "border-app-border"}`}
                aria-pressed={engineState === item.id}
              >
                <span className="block font-semibold">{item.label}</span>
                <span className="text-app-text-muted">{item.priority}</span>
              </button>
            ))}
          </div>
        </div>
        <div className="mt-4 rounded border border-app-border bg-app-surface-alt p-3">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h3 className="text-xs font-semibold uppercase tracking-wide text-app-text-muted">Debug state JSON</h3>
              <p className="mt-1 text-xs text-app-text-muted">Copy this snapshot when reporting state-machine behavior.</p>
            </div>
            <Button variant="outline" size="app-sm" onClick={() => void copyDebugState()}>
              Copy JSON
            </Button>
          </div>
          <pre className="mt-3 max-h-64 overflow-auto rounded border border-app-border bg-app-surface p-3 text-xs leading-relaxed text-app-text">{debugStateJson}</pre>
        </div>
      </section>
      <div className="mt-6 flex flex-wrap items-end gap-2">
        <select aria-label="Relation" value={relation} onChange={(event) => setRelation(event.target.value as (typeof relations)[number])}>
          {relations.map((item) => (
            <option key={item}>{item}</option>
          ))}
        </select>
        <select aria-label="Voice mode" value={voiceMode} onChange={(event) => setVoiceMode(event.target.value as "automatic" | "exact-response")}>
          <option value="automatic">Automatic</option>
          <option value="exact-response">Exact pace</option>
        </select>
        <label className="flex flex-col gap-1 text-xs text-app-text-muted">
          Sample delta (ms)
          <input
            aria-label="Sample delta (ms)"
            type="number"
            min="1"
            step="100"
            value={sampleDeltaMs}
            onChange={(event) => setSampleDeltaMs(Math.max(1, Number(event.target.value) || 1))}
            className="h-9 w-32 rounded border border-app-border bg-app-surface px-2 text-sm text-app-text"
          />
          <span>{sampleDeltaText(sampleDeltaMs)}</span>
        </label>
        <Button onClick={() => void previewPace()}>Preview pace</Button>
        <Button onClick={audio.stop}>Stop</Button>
      </div>
      <p className="mt-3 text-xs text-app-text-muted">
        {audio.catalog
          ? `${audio.visibleLines.length} Qwen clips · ${audio.catalog.model ?? "Qwen"} · validation ${audio.catalog.validation ? "passed" : "pending"} · ${audio.catalog.fullLines.length} Qwen full lines`
          : "Loading Qwen audio catalog…"}
      </p>
      <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
        <section className="mt-6 space-y-5">
          <div>
            <h3 className="font-semibold">Sentences</h3>
            <p className="mt-1 text-xs text-app-text-muted">Complete examples sent to drivers, including lap-time deltas.</p>
            <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              {sentenceExamples.map((example) => (
                <div key={example.id} className="flex items-center justify-between gap-3 rounded border border-app-border bg-app-surface-alt p-3">
                  <div>
                    <div className="font-medium">{renderedSentenceText[example.id] ?? example.label.replace("point three seconds", sampleDeltaText(sampleDeltaMs))}</div>
                    <div className="mt-1 text-xs text-app-text-muted">
                      {example.kind === "lap-time"
                        ? "completed lap time · sample 1:32.417"
                        : example.kind === "opponent-lap-pace"
                          ? "opponent lap pace · sample delta 0.300s"
                          : example.kind === "preview-line"
                            ? "dev preview line"
                            : `${"relation" in example ? example.relation : ""} · sample delta ${sampleDeltaText(sampleDeltaMs)}`}
                    </div>
                  </div>
                  <Button variant="app-outline" size="app-sm" aria-label={`Play Qwen sentence: ${example.label}`} onClick={() => void playSentence(example)}>
                    Play Qwen
                  </Button>
                </div>
              ))}
          ...
          <div>
            <h3 className="font-semibold">Individual Qwen clips</h3>
            <p className="mt-1 text-xs text-app-text-muted">Atomic Qwen voice clips used to build complete sentences.</p>
            <div className="mt-3 space-y-5">
              {groups.map(([label, clips]) => (
                <div key={label}>
                  <h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-app-text-muted">{label}</h4>
                  <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                    {clips.map((clip) => (
                      <div key={clip.segmentId} className="flex items-center gap-3 rounded border border-app-border bg-app-surface-alt p-3">
                        <div className="min-w-0 flex-1">
                          <div className="font-medium truncate">{clip.spokenText || clip.segmentId}</div>
                          <div className="mt-1 text-xs text-app-text-muted">
                            {clip.segmentId} · {clip.durationMs} ms
                          </div>
                        </div>
                        <Button
                          variant="app-outline"
                          size="icon-sm"
                          aria-label={`Play Qwen clip: ${clip.spokenText || clip.segmentId}`}
                          onClick={() => void audio.playQwenClip(`qwen-clip-${clip.segmentId}`, clip.segmentId)}
                        >
        ...
        {audio.result && (
          <aside className="order-first h-fit self-start rounded border border-app-border bg-app-surface-alt p-4 lg:sticky lg:top-4 lg:order-last lg:max-h-[calc(100vh-2rem)] lg:overflow-auto">
            <h3 className="font-semibold">Render JSON</h3>
            <pre className="mt-3 max-h-[32rem] overflow-auto whitespace-pre-wrap text-xs">{JSON.stringify(audio.result, null, 2)}</pre>
          </aside>
        )}
      </div>
    </div>
