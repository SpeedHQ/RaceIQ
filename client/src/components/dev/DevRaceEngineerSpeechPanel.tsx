...
export function DevRaceEngineerSpeechPanel() {
  ...
  const playSentence = async (example: (typeof sentenceExamples)[number], source: "omnivoice" | "qwen") => {
  ...
  return <div className="h-full overflow-y-auto p-6">
    ...
    <section className="mt-6 space-y-5"><div><h3 className="font-semibold">Sentences</h3><p className="mt-1 text-xs text-app-text-muted">Complete examples sent to drivers, including lap-time deltas.</p><div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{sentenceExamples.map((example) => <div key={example.id} className="flex items-center justify-between gap-3 rounded border border-app-border bg-app-surface-alt p-3"><div><div className="font-medium">{renderedSentenceText[example.id] ?? example.label.replace("point three seconds", sampleDeltaText(sampleDeltaMs))}</div><div className="mt-1 text-xs text-app-text-muted">{example.kind === "lap-time" ? "completed lap time · sample 1:32.417" : `${example.relation} · sample delta ${sampleDeltaText(sampleDeltaMs)}`}</div></div><div className="flex shrink-0 gap-1"><Button variant="app-ghost" size="app-sm" aria-label={`Play OmniVoice sentence: ${example.label}`} onClick={() => void playSentence(example, "omnivoice")}>OmniVoice</Button><Button variant="app-outline" size="app-sm" aria-label={`Play Qwen sentence: ${example.label}`} onClick={() => void playSentence(example, "qwen")}>Qwen</Button></div></div>)}</div></div>
      <div><h3 className="font-semibold">Individual clips</h3><p className="mt-1 text-xs text-app-text-muted">Atomic voice clips used to build complete sentences.</p><div className="mt-3 space-y-5">{groups.map(([label, clips]) => <div key={label}><h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-app-text-muted">{label}</h4><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{clips.map((clip) => <div key={clip.segmentId} className="flex items-center gap-3 rounded border border-app-border bg-app-surface-alt p-3"><div className="min-w-0 flex-1"><div className="font-medium truncate">{clip.spokenText || clip.segmentId}</div><div className="mt-1 text-xs text-app-text-muted">{clip.segmentId} · {clip.durationMs} ms</div></div><div className="flex shrink-0 gap-1"><Button variant="app-ghost" size="icon-sm" aria-label={`Play OmniVoice clip: ${clip.spokenText || clip.segmentId}`} onClick={() => void audio.playSegments(`omnivoice-clip-${clip.segmentId}`, [clip.segmentId])}><span aria-hidden="true">O</span></Button><Button variant="app-outline" size="icon-sm" aria-label={`Play Qwen clip: ${clip.spokenText || clip.segmentId}`} onClick={() => void audio.playQwenClip(`qwen-clip-${clip.segmentId}`, clip.segmentId)}><span aria-hidden="true">Q</span></Button></div><audio controls preload="none" src={`/audio/live-engineer/v1/${clip.path}`} className="hidden" /></div>)}</div></div>)}</div></div>
    </section>
