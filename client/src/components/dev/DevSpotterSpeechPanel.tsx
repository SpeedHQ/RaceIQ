...
function SpeechCatalog({ audio }: { audio: ReturnType<typeof useDevSpeechAudio> }) { return <><p className="mt-3 text-xs text-app-text-muted">{audio.catalog ? `${audio.visibleLines.length} clips · ${audio.catalog.pipelineVersion ?? "no generated catalog"} · validation ${audio.catalog.validation ? "passed" : "pending"}` : "Loading audio catalog…"}</p><div className="mt-6 overflow-hidden rounded border border-app-border"><div className="grid grid-cols-[minmax(12rem,1fr)_minmax(12rem,1fr)_8rem] gap-3 border-b border-app-border bg-app-surface-alt px-3 py-2 text-xs font-semibold"><span>Line</span><span>Audio</span><span>Status</span></div>{audio.visibleLines.map((clip) => <div key={clip.segmentId} className="grid grid-cols-[minmax(12rem,1fr)_minmax(12rem,1fr)_8rem] items-center gap-3 border-b border-app-border px-3 py-3 text-xs"><div><div className="font-medium">{clip.segmentId}</div><div className="text-app-text-muted">{clip.spokenText}</div></div><audio controls preload="none" src={`/audio/live-engineer/v1/${clip.path}`} className="h-8 w-full" /><div className="text-app-text-muted">{clip.durationMs} ms</div></div>)}</div></>; }
