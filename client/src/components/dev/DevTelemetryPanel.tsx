...
export function DevTelemetryPanel() {
  ...
  return <div className="flex h-full flex-col overflow-hidden"><div className="flex items-center justify-between border-b border-app-border p-4"><div><h2 className="font-semibold">Native Telemetry</h2><span className="text-xs text-app-text-muted">{status}</span></div><Button variant="app-outline" size="app-sm" type="button" onClick={() => setWanted(!wanted)}>{wanted ? "Stop" : "Start"}</Button></div><div className="min-h-0 flex-1"><RawTelemetry packet={packet} /></div></div>;
}
