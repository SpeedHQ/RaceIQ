...
import type {
  TrackImageryLayerKind,
  TrackImageryLayoutManifest,
  TrackImagerySource,
...
function useImageryBaseForm(venue: TrackImageryVenueManifest | null, assetVersion: number) {
  ...
  const persistedPreviewUrl = venue
    ? `/api/dev/track-imagery/venues/texture/base?venueId=${encodeURIComponent(venue.venueId)}&v=${assetVersion}`
    : null;
