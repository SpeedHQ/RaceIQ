      ...
      <FieldGroup className="gap-2">
        {(["name", "url", "capturedAt", "license", "attribution"] as const).map((key) => (
          <Field key={key} className="gap-0.5">
            <FieldLabel className="text-app-compact font-medium capitalize text-app-text-muted" htmlFor={`${title}-${key}`}>
...
export function ImageryBaseEditor({
  ...
  return (
    <section className="mb-4 rounded border border-app-border p-3">
      ...
      />
      {estimating && <p className="mb-2 text-app-compact text-app-text-muted">Calculating width, pixels, tiles, work, pack size, disk, duration, and concurrency…</p>}
    ...
    <>
      ...
      <section className="mb-4 rounded border border-app-border p-3">
        ...
        <div className="mb-2 grid grid-cols-2 gap-2">
          ...
          <label className="text-app-compact text-app-text-muted">
  ...
  return (
    <>
      <div className="mb-3 rounded border border-app-border bg-app-surface-alt p-2">
        <div className="text-app-caption uppercase tracking-wide text-app-text-muted">Assigned venue</div>
        {configuration ? (
          <>
            <div className="mt-1 text-xs text-app-text">{[configuration.venue.name, ...configuration.subVenues.map((entry) => entry.name)].join(" / ")}</div>
            <div className="font-mono text-app-caption text-app-text-muted">{venueId}</div>
          </>
        ) : (
      ...
      {calibration.lapId !== null && calibration.replay?.georeference ? (
        ...
        <p className="text-xs text-app-text-muted">
          GPS: {catalogReferenceMatchLabel(calibration.catalogReference.match)} match · {calibration.catalogReference.outlineSource} outline
        </p>
      ) : (
