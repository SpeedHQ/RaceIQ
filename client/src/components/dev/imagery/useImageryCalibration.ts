...
import { useLapSemanticTelemetry, useLaps, type SemanticLapTelemetry } from "../../../hooks/laps";

type CalibrationDragMode = "move" | "rotate" | "scale";

type ImageryCalibrationReplay = SemanticLapTelemetry & {
  geographicPositions?: (TrackImageryGeographicPoint | null)[];
  georeference?: {
    kind: "native" | "derived";
    quality: { rmseM: number };
  };
};
...
export function useImageryCalibration({ gameId, trackOrdinal, catalogReference, catalogReferenceLoading = false, initialCalibration, baseUrl, selectedCandidate }: UseImageryCalibrationOptions) {
  ...
  const { data: replayData, isLoading: replayLoading } = useLapSemanticTelemetry(lapId);
  const replay = replayData as ImageryCalibrationReplay | undefined;
