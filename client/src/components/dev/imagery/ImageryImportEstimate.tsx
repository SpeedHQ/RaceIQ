...
export function ImageryImportEstimate({ budget }: { budget: TrackImageryOutputBudget }) {
  return (
    <div
      role="status"
      className={cn("mb-2 rounded border p-2 text-app-compact", budget.safe ? "border-app-border bg-app-surface-alt text-app-text-muted" : "border-severity-critical text-severity-critical")}
    >
