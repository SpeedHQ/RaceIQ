...
import { TRACK_IMAGERY_MANIFEST_VERSION, type TrackImageryCandidate, type TrackImageryVenueManifest } from "../../../../shared/racing/tracks/imagery";
...
export function TrackImageryCalibrationPanel({ selection, configurationRevision }: { selection: TrackConfigurationSelection; configurationRevision: number }) {
  ...
  const displayedLayers = useMemo(() => pack.venue?.layers.filter((layer) => forms.layer.selectedIds.includes(layer.id)) ?? [], [forms.layer.selectedIds, pack.venue]);
  ...
  const saveBase = async () => {
    ...
    try {
      ...
      importJob.clear();
      setStatus(selectedCandidate ? `${selectedCandidate.quality === "hq" ? "HQ" : "Context fallback"} open imagery imported and assigned.` : "Opaque venue base and layout assignment saved.");
    } catch (saveError) {
    ...
    <div className="grid h-full min-h-0 grid-cols-1 bg-app-bg @7xl/workspace:grid-cols-[minmax(19rem,25rem)_minmax(0,1fr)]">
      <aside className="min-h-0 overflow-y-auto border-r border-app-border p-4">
        ...
        <ImageryPackStatus configuration={pack.configuration} venueId={pack.venueId} calibration={calibration} status={visibleStatus} error={error}>
          ...
          />
          {pack.venue && <ImageryLayersEditor venue={pack.venue} form={forms.layer} saving={saving} onSaveStack={() => void saveLayerStack()} onSaveLayer={() => void saveLayer()} />}
        </ImageryPackStatus>
      </aside>
