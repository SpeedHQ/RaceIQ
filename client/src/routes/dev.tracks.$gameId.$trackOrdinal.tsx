    ...
    <div className="flex min-h-full flex-col">
      <div className="border-b border-app-border bg-app-surface px-4 py-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="font-mono text-xs text-app-text-muted">{gameId} · ordinal {trackOrdinal}</p>
            <h2 className="text-lg font-semibold text-app-text">{catalogTrack?.name ?? "Selected track workbench"}</h2>
            <p className="text-sm text-app-text-muted">{catalogTrack?.variant || "Main"}{configuration ? ` · ${trackConfigurationCanonicalId(configuration)}` : ""}</p>
            {configuration?.confirmation ? (
              <p className="text-xs text-app-text-muted">Confirmed {configuration.confirmation.confirmedAt} by {configuration.confirmation.confirmedBy}</p>
            ) : (
