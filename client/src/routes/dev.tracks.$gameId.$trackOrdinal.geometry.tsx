...
function GeometryRoute() {
  ...
  if (trackQuery.error) {
    return (
      <Alert variant="destructive" className="m-4">
        <AlertTitle>Unable to load geometry</AlertTitle>
        <AlertDescription>{trackQuery.error.message}</AlertDescription>
      </Alert>
    );
  }
    ...
    return (
      <Empty className="m-4">
        <EmptyHeader>
          ...
          <EmptyDescription>Selected catalog row is unavailable.</EmptyDescription>
        </EmptyHeader>
      </Empty>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-3 p-4">
      <DevTrackGeometryPage gameId={gameId} track={track} mode={mode} onModeChange={(nextMode) => void navigate({ search: { mode: nextMode } })} />
    </div>
  );
}
