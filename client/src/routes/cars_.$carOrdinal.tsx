...
/** Static packet for showcase — car stationary, neutral position */
...
function CarModelPage() {
  ...
  const { data: carInfo } = useQuery({
    ...
    queryFn: () => fetch(`/api/cars/${ordinal}`).then(r => r.ok ? r.json() : null),
  });
...
export const Route = createFileRoute("/cars_/$carOrdinal")({
