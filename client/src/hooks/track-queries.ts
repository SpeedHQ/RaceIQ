...
export function useTrackImagery(ord: number | undefined, gameIdOverride?: GameId | null) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await client.api["track-imagery"][":ordinal"].$get({
        param: { ordinal: String(ord!) },
        query: { gameId: gameId! },
      });
      return rpcJson<TrackImagery | null>(response);
    },
    enabled: ord != null && ord >= 0 && !!gameId,
    staleTime: Number.POSITIVE_INFINITY,
  });
...
export function useTrackImageryReference(ord: number | undefined, gameIdOverride?: GameId | null, enabledOverride = true) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await devClient.api.dev["track-imagery"].reference[":ordinal"].$get({
        param: { ordinal: String(ord!) },
        query: { gameId: gameId! },
      });
...
export function useTrackCurbs(ord: number | undefined, gameIdOverride?: GameId | null) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await client.api["track-curbs"][":ordinal"].$get({
        param: { ordinal: String(ord!) },
...
export function useTrackTimingSectorLayout({ gameId, trackOrdinal }: { gameId: GameId | null; trackOrdinal: number }) {
