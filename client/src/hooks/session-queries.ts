...
export function useSessionRuns(
  ...
  return useInfiniteQuery({
    queryKey: queryKeys.sessionRuns(sessionId ?? null, gameId ?? null, query),
    queryFn: async ({ pageParam }) => {
      if (sessionId == null) {
        throw new Error("useSessionRuns: sessionId is required");
      }
      if (!gameId) {
        throw new Error("useSessionRuns: gameId is required");
      }
      const response = await client.api.sessions[":id"].runs.$get({
        param: { id: String(sessionId) },
        query: { gameId, ...sessionRunQueryParams(query, pageParam) },
      });
