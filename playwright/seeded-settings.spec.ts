  ...
  try {
    ...
    expect(
      browserErrors.errors.filter(
        (error) =>
          !error.includes("/api/storage/sessions") &&
          error !== "console.error: Failed to load resource: the server responded with a status of 503 (Service Unavailable)",
      ),
    ).toEqual([]);
  } finally {
    await page.unroute("**/api/storage/sessions");
