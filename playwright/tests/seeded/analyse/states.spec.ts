...
test("Analyse exposes loading and parse-error states", async ({ page, request }) => {
  ...
  await page.goto(`/fm23/sessions/replay?track=${target.trackOrdinal}&car=${target.carOrdinal}&lap=${target.id}`, { waitUntil: "domcontentloaded" });
...
test("Analyse renders seeded no-telemetry lap state when available", async ({ page, request }) => {
  ...
  if (!emptyLap) return;
  await page.goto(`/fm23/sessions/replay?track=${emptyLap.trackOrdinal}&car=${emptyLap.carOrdinal}&lap=${emptyLap.id}`, { waitUntil: "domcontentloaded" });
