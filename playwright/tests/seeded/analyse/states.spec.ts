...
test("Analyse exposes loading and parse-error states", async ({ page, request }) => {
  ...
  await page.goto(`/fm23/sessions/${target.sessionId}/replay/${target.id}`, { waitUntil: "domcontentloaded" });
...
test("Analyse renders seeded no-telemetry lap state when available", async ({ page, request }) => {
  ...
  if (!emptyLap) return;
  await page.goto(`/fm23/sessions/${emptyLap.sessionId}/replay/${emptyLap.id}`, { waitUntil: "domcontentloaded" });
