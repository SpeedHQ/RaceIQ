...
export async function exportImportAndDelete(page: Page, request: APIRequestContext, selectedLap: LapMeta, selectedLapId: number, importedLapIds: number[]): Promise<void> {
  ...
  await page.goto(`/fm23/sessions/analyse?track=${disposableLap.trackOrdinal}&car=${disposableLap.carOrdinal}&lap=${disposableLap.id}`, { waitUntil: "domcontentloaded" });
  ...
  await page.goto(`/fm23/sessions/analyse?track=${selectedLap.trackOrdinal}&car=${selectedLap.carOrdinal}&lap=${selectedLapId}`, { waitUntil: "domcontentloaded" });
  await expect(page.getByRole("heading", { name: "Metrics at Cursor" })).toBeVisible({ timeout: 30_000 });
}
