...
const BASE_FEATURES = [
  ...
  "track-info",
  "track-laps",
] as const satisfies readonly SeededFeature[];
...
function routeFor(game: SeededGame, feature: SeededFeature): Omit<SeededRouteCase, "game" | "feature" | "label"> {
  switch (feature) {
    ...
    case "live":
      return game.prefix === "iracing"
        ? { path: "/iracing/live", expectedPath: "/iracing/live/driver" }
        : { path: `/${game.prefix}/live` };
    case "track-info":
