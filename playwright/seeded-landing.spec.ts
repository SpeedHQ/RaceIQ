...
test("global landing cards, period filters, and recent laps navigate", async ({
  page,
}) => {
  ...
  for (const game of SEEDED_GAME_CASES) {
    await expect(page.locator(`a[data-game-brand="${game.gameId}"]`)).toHaveAttribute(
      "href",
      `/${game.prefix}`,
    );
  }
...
for (const game of SEEDED_GAME_CASES) {
  test(`${game.name} landing primary action navigates with lap data`, async ({
    page,
    request,
  }) => {
    const browserErrors = collectBrowserErrors(page);
    ...
    const recapResponse = await request.get(
      `/api/sessions/${latestSession.id}/recap?gameId=${game.gameId}`,
    );
    ...
    await expect(page).toHaveURL(
      new RegExp(`/${game.prefix}/analyse\\?[^#]*lap=${recap.bestLapId}(?:&|$)`),
    );
    await expect(page.getByRole("heading", { name: "Metrics at Cursor" })).toBeVisible({
      timeout: 20_000,
    });
